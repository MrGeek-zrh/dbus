...
BusConfigParser *bus_config_parser_new(const DBusString *basedir, dbus_bool_t is_toplevel,
...
void bus_config_parser_unref(BusConfigParser *parser)
{
    ...
    if (parser->refcount == 0) {
        ...
        _dbus_list_clear_full(&parser->service_dirs, (DBusFreeFunction)bus_config_service_dir_free);
...
static dbus_bool_t locate_attributes(BusConfigParser *parser, const char *element_name, const char **attribute_names,
                                     const char **attribute_values, DBusError *error, const char *first_attribute_name,
    ...
    while (attribute_names[i]) {
        ...
        while (j < n_attrs) {
            if (strcmp(attrs[j].name, attribute_names[i]) == 0) {
                ...
                if (*retloc != NULL) {
                    dbus_set_error(error, DBUS_ERROR_FAILED, "Attribute \"%s\" repeated twice on the same <%s> element",
        ...
        if (!found) {
            dbus_set_error(error, DBUS_ERROR_FAILED, "Attribute \"%s\" is invalid on <%s> element in this context",
...
static dbus_bool_t check_no_attributes(BusConfigParser *parser, const char *element_name, const char **attribute_names,
                                       const char **attribute_values, DBusError *error)
{
...
static dbus_bool_t start_busconfig_child(BusConfigParser *parser, const char *element_name,
                                         const char **attribute_names, const char **attribute_values, DBusError *error)
{
    ...
    if (element_type == ELEMENT_USER) {
       ...
       * them, but we do still parse them) */
        ...
        if (!locate_attributes(parser, "include", attribute_names, attribute_values, error, "ignore_missing",
                               &ignore_missing, "if_selinux_enabled", &if_selinux_enabled, "selinux_root_relative",
        ...
        if (!locate_attributes(parser, "policy", attribute_names, attribute_values, error, "context", &context, "user",
                               &user, "group", &group, "at_console", &at_console, NULL))
            return FALSE;
        ...
        if (context != NULL) {
            ...
            if (t || strcmp(at_console, "false") == 0) {
                ...
                dbus_set_error(error, DBUS_ERROR_FAILED,
                               "Unknown value \"%s\" for at_console in message bus configuration file", at_console);
        ...
        dbus_set_error(error, DBUS_ERROR_FAILED, "Element <%s> not allowed inside <%s> in configuration file",
 ...
 */
static dbus_bool_t parse_int_attribute(const char *name, const char *content, long min, long max, long def, long *value,
                                       DBusError *error)
{
...
static dbus_bool_t append_rule_from_element(BusConfigParser *parser, const char *element_name,
                                            ...
                                            dbus_bool_t allow, DBusError *error)
{
    ...
    if (!locate_attributes(parser, element_name, attribute_names, attribute_values, error, "send_interface",
                           &send_interface, "send_member", &send_member, "send_error", &send_error, "send_destination",
                           &send_destination, "send_destination_prefix", &send_destination_prefix, "send_path",
                           &send_path, "send_type", &send_type, "send_broadcast", &send_broadcast, "receive_interface",
                           &receive_interface, "receive_member", &receive_member, "receive_error", &receive_error,
                           "receive_sender", &receive_sender, "receive_path", &receive_path, "receive_type",
                           &receive_type, "eavesdrop", &eavesdrop, "max_fds", &max_fds_attr, "min_fds", &min_fds_attr,
                           "send_requested_reply", &send_requested_reply, "receive_requested_reply",
                           &receive_requested_reply, "own", &own, "own_prefix", &own_prefix, "user", &user, "group",
                           &group, "log", &log, NULL))
        return FALSE;

    any_send_attribute = (send_destination != NULL || send_destination_prefix != NULL || send_broadcast != NULL ||
                          send_path != NULL || send_type != NULL || send_interface != NULL || send_member != NULL ||
                          send_error != NULL || send_requested_reply != NULL);
    any_receive_attribute = (receive_sender != NULL || receive_path != NULL || receive_type != NULL ||
                             receive_interface != NULL || receive_member != NULL || receive_error != NULL ||
    ...
    any_message_attribute = (any_send_attribute || any_receive_attribute || eavesdrop != NULL || max_fds_attr != NULL ||
                             min_fds_attr != NULL);
...
#define IS_WILDCARD(str) ((str) && ((str)[0]) == '*' && ((str)[1]) == '\0')
    ...
    if (any_send_attribute) {
        ...
        if (eavesdrop && !(strcmp(eavesdrop, "true") == 0 || strcmp(eavesdrop, "false") == 0)) {
            dbus_set_error(error, DBUS_ERROR_FAILED, "Bad value \"%s\" for %s attribute, must be true or false",
                           "eavesdrop", eavesdrop);
            return FALSE;
        }
       ...
       * but perhaps an unrealistically high one. */
        if (!parse_int_attribute("max_fds", max_fds_attr, 0, DBUS_MAXIMUM_MESSAGE_UNIX_FDS,
        ...
        switch (pe->d.policy.type) {
            ...
            case POLICY_USER:
                if (!BUS_POLICY_RULE_IS_PER_CLIENT(rule)) {
                    dbus_set_error(error, DBUS_ERROR_FAILED,
                                   "<%s> rule cannot be per-user because it has bus-global semantics", element_name);
                    goto failed;
                }

                if (!bus_policy_append_user_rule(parser->policy, pe->d.policy.gid_uid_or_at_console, rule))
                    goto nomem;
                break;
            case POLICY_GROUP:
                if (!BUS_POLICY_RULE_IS_PER_CLIENT(rule)) {
                    dbus_set_error(error, DBUS_ERROR_FAILED,
                                   "<%s> rule cannot be per-group because it has bus-global semantics", element_name);
                    goto failed;
                }

                if (!bus_policy_append_group_rule(parser->policy, pe->d.policy.gid_uid_or_at_console, rule))
                    goto nomem;
                break;
...
static dbus_bool_t start_policy_child(BusConfigParser *parser, const char *element_name, const char **attribute_names,
                                      const char **attribute_values, DBusError *error)
{
...
static dbus_bool_t start_selinux_child(BusConfigParser *parser, const char *element_name, const char **attribute_names,
                                       const char **attribute_values, DBusError *error)
{
    char *own_copy;
    char *context_copy;
    ...
    if (strcmp(element_name, "associate") == 0) {
        ...
        if (!locate_attributes(parser, "associate", attribute_names, attribute_values, error, "own", &own, "context",
                               &context, NULL))
            return FALSE;
        ...
        if (!_dbus_hash_table_insert_string(parser->service_context_table, own_copy, context_copy))
            goto oom;
                                            ...
                                            DBusError *error)
{
    ...
    if (t == ELEMENT_NONE) {
        if (strcmp(element_name, "busconfig") == 0) {
            ...
            dbus_set_error(error, DBUS_ERROR_FAILED, "Unknown element <%s> at root of configuration file",
                           element_name);
            return FALSE;
        }
...
dbus_bool_t bus_config_parser_end_element(BusConfigParser *parser, const char *element_name, DBusError *error)
{
    ...
    if (strcmp(n, element_name) != 0) {
       ...
       * being paranoid about XML parsers
       */
        dbus_set_error(error, DBUS_ERROR_FAILED, "XML element <%s> ended but topmost element on the stack was <%s>",
                       element_name, n);
        return FALSE;
    }
    ...
    switch (e->type) {
        ...
        case ELEMENT_LIMIT:
            ...
            if (e->type == ELEMENT_LIMIT) {
                if (!set_limit(parser, e->d.limit.name, e->d.limit.value, error))
                    return FALSE;
            }
            break;
...
static dbus_bool_t include_file(BusConfigParser *parser, const DBusString *filename, dbus_bool_t ignore_missing,
                                DBusError *error)
{
            ...
            if (!include_file(parser, &full_path, TRUE, error)) {
                if (dbus_error_is_set(error)) {
                   ...
                   * somehow. */
                    ...
                    _dbus_log(DBUS_SYSTEM_LOG_INFO, "Encountered error '%s' while parsing '%s'", error->message,
...
dbus_bool_t bus_config_parser_finished(BusConfigParser *parser, DBusError *error)
{
    ...
    if (parser->stack != NULL) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "Element <%s> was not closed in configuration file",
                       bus_config_parser_element_type_to_name(top_element_type(parser)));
...
static dbus_bool_t elements_equal(const Element *a, const Element *b)
{
    ...
    switch (a->type) {
        case ELEMENT_INCLUDE:
            if (!bools_equal(a->d.include.ignore_missing, b->d.include.ignore_missing))
                return FALSE;
            break;
...
{
    return (a->max_incoming_bytes == b->max_incoming_bytes && a->max_incoming_unix_fds == b->max_incoming_unix_fds &&
   ...
   * right, the XDG_DATA_DIRS will be too. */
    ...
    for (link = _dbus_list_get_first_link(dirs), i = 0; link != NULL;
        ...
        if (strcmp(test_session_service_dir_matches[i], dir->path) != 0) {
            _dbus_test_diag("'%s' directory does not match '%s' in the match set", dir->path,
       ...
       * in the XDG_RUNTIME_DIR, and we expect it to have special flags */
        if (i == 0)
            expected = (BUS_SERVICE_DIR_FLAGS_NO_WATCH | BUS_SERVICE_DIR_FLAGS_STRICT_NAMING);
#endif
   ...
   * the first one is transient */
    ...
    for (link = _dbus_list_get_first_link(&watched_dirs); link != NULL;
        ...
        if (strcmp(test_session_service_dir_matches[i], (const char *)link->data) != 0) {
            _dbus_test_diag("'%s' directory does not match '%s' in the match set", (const char *)link->data,
                            test_session_service_dir_matches[i]);
            goto out;
        }
    }
...
#ifndef DBUS_WIN
static const char *test_system_service_dir_matches[] = { "/usr/local/share/dbus-1/system-services",
...
static dbus_bool_t test_default_system_servicedirs(void)
{
    ...
    while ((link = _dbus_list_pop_first_link(&dirs))) {
        ...
        if (strcmp(test_system_service_dir_matches[i], (char *)link->data) != 0) {
            _dbus_test_diag("%s directory does not match %s in the match set", (char *)link->data,
