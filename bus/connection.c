...
typedef struct {
    ...
    // 指向 BusConnections 结构体的指针，表示与总线连接的所有连接。

    DBusList *link_in_connection_list;
    // 指向连接列表中当前连接的指针，用于维护连接列表中的位置。

    DBusConnection *connection;
    // 指向 DBusConnection 结构体的指针，表示当前的 DBus 连接。
    ...
    // 指向 BusClientPolicy 结构体的指针，表示客户端策略。

    char *cached_loginfo_string;
    // 缓存的日志信息字符串。

    BusSELinuxID *selinux_id;
    // 指向 SELinux ID 结构体的指针，表示 SELinux 安全上下文。

    BusAppArmorConfinement *apparmor_confinement;
    // 指向 AppArmor 限制结构体的指针，表示 AppArmor 安全上下文。
    ...
    int stamp;
    /**< 上次遍历时 connections->stamp 的值 */

    BusExtraHeaders want_headers;
    // 额外的头信息，表示此连接希望接收的额外头信息。
...
static dbus_bool_t adjust_connections_for_uid(BusConnections *connections, dbus_uid_t uid, int adjustment)
{
    ...
    _dbus_verbose("Adjusting connection count for UID " DBUS_UID_FORMAT ": was %d adjustment %d making %d\n", uid,
                  current_count, adjustment, current_count + adjustment);
    ...
    if (current_count == 0) {
        ...
        retval = _dbus_hash_table_insert_uintptr(connections->completed_by_user, uid,
...
// 用于检查给定的 Unix 用户 ID (uid) 是否被允许连接到 D-Bus 总线。
...
static dbus_bool_t cache_peer_loginfo_string(BusConnectionData *d, DBusConnection *connection)
{
    ...
    if (pid != DBUS_PID_UNSET) {
        ...
        if (!_dbus_string_append_printf(&loginfo_buf, "pid=" DBUS_PID_FORMAT " comm=\"", pid))
            goto oom;
    ...
    if (bus_containers_connection_is_contained(connection, &container, &container_type, &container_name)) {
        ...
        did_append = _dbus_string_append_printf(&loginfo_buf, "container=%s %s=\"%s\")", container, container_type,
                                                container_name);
        if (!did_append)
            goto oom;
        else
...
/**
 * @brief 检查和处理 DBusConnection 上挂起的文件描述符
 *
 * 这个函数用于在挂起的文件描述符数量变化时，启用或禁用相关的超时机制。
 *
 * @param connection 指向 DBusConnection 结构体的指针
 */
static void check_pending_fds_cb(DBusConnection *connection)
{
    ...
    // 如果旧数量大于 0 且新数量为 0，则禁用挂起文件描述符的超时机制
    if (n_pending_unix_fds_old > 0 && n_pending_unix_fds_new == 0) {
...
static dbus_bool_t pending_unix_fds_timeout_cb(void *data)
{
    ...
    bus_context_log(d->connections->context, DBUS_SYSTEM_LOG_WARNING,
                    ...
                    d->name != NULL ? d->name : "(null)", bus_connection_get_loginfo(connection), limit);
...
dbus_bool_t bus_connections_setup_connection(BusConnections *connections, DBusConnection *connection)
{
    ...
    // 设置是否路由 org.freedesktop.DBus.Peer 消息
    ...
    if (dbus_error_is_set(&error)) {
        bus_context_log(connections->context, DBUS_SYSTEM_LOG_WARNING, "Unable to set up new connection: %s",
                        error.message);
        dbus_error_free(&error);
        goto error;
    }
    ...
    // 设置超时函数
    if (!dbus_connection_set_timeout_functions(connection, add_connection_timeout, remove_connection_timeout, NULL,
                                               connection, NULL))
        goto oom;
    ...
    // 设置挂起的文件描述符函数
    _dbus_connection_set_pending_fds_function(connection, (DBusPendingFdsChangeFunction)check_pending_fds_cb,
                                              connection);
...
dbus_bool_t bus_connection_get_unix_groups(DBusConnection *connection, unsigned long **groups, int *n_groups,
                                           DBusError *error)
{
   ...
   * they are the same type) */
    ...
    *n_groups = 0;
    ...
    if (credentials != NULL && _dbus_credentials_get_unix_gids(credentials, &groups_borrowed, &n)) {
        size_t i;
...
dbus_bool_t bus_connection_is_in_unix_group(DBusConnection *connection, unsigned long gid)
{
    ...
    if (!bus_connection_get_unix_groups(connection, &group_ids, &n_group_ids, NULL))
        return FALSE;
 ...
 */
dbus_bool_t bus_connection_is_queued_owner_by_prefix(DBusConnection *connection, const char *name_prefix)
{
    BusConnectionData *d;
    DBusList *link;
...
dbus_bool_t bus_connection_complete(DBusConnection *connection, const DBusString *name, DBusError *error)
{
    ...
    d->policy = bus_context_create_client_policy(d->connections->context, connection, NULL, error);
...
dbus_bool_t bus_connections_reload_policy(BusConnections *connections, DBusError *error)
{
    ...
    for (link = _dbus_list_get_first_link(&(connections->completed)); link;
        ...
        policy = bus_context_create_client_policy(connections->context, connection, d->policy, error);
        if (policy == NULL) {
            _dbus_verbose("Failed to create security policy for connection %p\n", connection);
            _DBUS_ASSERT_ERROR_IS_SET(error);
            return FALSE;
 ...
 */
dbus_bool_t bus_connections_check_limits(BusConnections *connections, DBusConnection *requesting_completion,
                                         const char **limit_name_out, int *limit_out, DBusError *error)
{
    unsigned long uid;
    int limit;
    ...
    if (dbus_connection_get_unix_user(requesting_completion, &uid)) {
        ...
        if (get_connections_for_uid(connections, uid) >= limit) {
            ...
            dbus_set_error(error, DBUS_ERROR_LIMITS_EXCEEDED,
                           "The maximum number of active connections for UID %lu has been reached", uid);
            return FALSE;
        }
    }
...
static dbus_bool_t bus_pending_reply_send_no_reply(BusConnections *connections, BusTransaction *transaction,
                                                   BusPendingReply *pending)
{
    ...
    if (!bus_transaction_send_from_driver(transaction, pending->will_get_reply, message))
        goto out;
    ...
    while (link != NULL) {
        ...
        if (pending->will_get_reply == connection) {
            ...
            _dbus_verbose("Dropping pending reply %p, replier %p receiver %p serial %u\n", pending,
                          pending->will_send_reply, pending->will_get_reply, pending->reply_serial);
 ...
 */
dbus_bool_t bus_connections_expect_reply(BusConnections *connections, BusTransaction *transaction,
                                         ...
                                         DBusMessage *reply_to_this, DBusError *error)
{
    ...
    if (count >= limit) {
        bus_context_log(connections->context, DBUS_SYSTEM_LOG_WARNING,
                        ...
                        bus_connection_get_name(will_get_reply), bus_connection_get_loginfo(will_get_reply), limit);
 ...
 */
dbus_bool_t bus_connections_check_reply(BusConnections *connections, BusTransaction *transaction,
                                        DBusConnection *sending_reply, DBusConnection *receiving_reply,
                                        DBusMessage *reply, DBusError *error)
{
    ...
    if (!bus_transaction_add_cancel_hook(transaction, cancel_check_pending_reply, cprd,
...
dbus_bool_t bus_transaction_capture_error_reply(BusTransaction *transaction, DBusConnection *addressed_recipient,
...
dbus_bool_t bus_transaction_send_from_driver(BusTransaction *transaction, DBusConnection *connection,
                                             DBusMessage *message)
{
   ...
   */
    if (!bus_context_check_security_policy(bus_transaction_get_context(transaction), transaction, NULL, connection,
                                           connection, message, NULL, &error)) {
...
dbus_bool_t bus_transaction_send(BusTransaction *transaction, DBusConnection *sender, DBusConnection *destination,
                                 DBusMessage *message)
{
    ...
    _dbus_verbose("  trying to add %s interface=%s member=%s error=%s to transaction%s\n",
                  ...
                  dbus_connection_get_is_connected(destination) ? "" : " (disconnected)");
   ...
   * So this is about the last possible time we could edit the header. */
    if ((d->want_headers & BUS_EXTRA_HEADERS_CONTAINER_INSTANCE) &&
        ...
        if (sender == NULL || !bus_containers_connection_is_contained(sender, &path, NULL, NULL))
            path = "/";
 ...
 */
dbus_bool_t bus_transaction_send_error_reply(BusTransaction *transaction, DBusConnection *connection,
                                             const DBusError *error, DBusMessage *in_reply_to)
{
    DBusMessage *reply;
...
dbus_bool_t bus_transaction_add_cancel_hook(BusTransaction *transaction, BusTransactionCancelFunction cancel_function,
...
void bus_connection_request_headers(DBusConnection *connection, BusExtraHeaders headers)
{
    BusConnectionData *d;
