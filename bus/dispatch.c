...
static dbus_bool_t send_one_message(DBusConnection *connection, BusContext *context, DBusConnection *sender,
                                    DBusConnection *addressed_recipient, DBusMessage *message,
                                    BusTransaction *transaction, DBusError *error)
{
    DBusError stack_error = DBUS_ERROR_INIT;

    if (!bus_context_check_security_policy(context, transaction, sender, addressed_recipient, connection, message, NULL,
                                           &stack_error)) {
...
dbus_bool_t bus_dispatch_matches(BusTransaction *transaction, DBusConnection *sender,
                                 DBusConnection *addressed_recipient, DBusMessage *message, DBusError *error)
{
    DBusError tmp_error;
   ...
   */
    ...
    if (addressed_recipient != NULL) {
        if (!bus_context_check_security_policy(context, transaction, sender, addressed_recipient, addressed_recipient,
                                               message, NULL, error))
            return FALSE;
        ...
        if (!bus_transaction_send(transaction, sender, addressed_recipient, message)) {
            BUS_SET_OOM(error);
            return FALSE;
        }
    }
    ...
    while (link != NULL) {
        ...
        if (!send_one_message(dest, context, sender, addressed_recipient, message, transaction, &tmp_error))
            break;
...
static void block_connection_until_message_from_bus(BusContext *context, DBusConnection *connection,
...
{
    while (dbus_connection_get_dispatch_status(connection) == DBUS_DISPATCH_NEED_MEMORY)
        _dbus_wait_for_memory();
...
static void warn_unexpected_real(DBusConnection *connection, DBusMessage *message, const char *expected,
...
static dbus_bool_t check_service_owner_changed_foreach(DBusConnection *connection, void *data)
{
    ...
    reget_service_info_data:
        ...
        dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &service_name, DBUS_TYPE_STRING, &old_owner,
        ...
        if (*service_name == ':' && new_owner[0] && strcmp(service_name, new_owner) != 0) {
            _dbus_warn("inconsistent ServiceOwnedChanged message (\"%s\" [ %s -> %s ])", service_name, old_owner,
                       new_owner);
            goto out;
        }
    }
...
static dbus_bool_t check_get_connection_unix_user(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(DBUS_SERVICE_DBUS, DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS,
 ...
 * but the correct thing may include OOM errors.
 */
static dbus_bool_t check_hello_connection(BusContext *context, DBusConnection *nil _DBUS_GNUC_UNUSED)
{
...
static dbus_bool_t check_nonexistent_service_no_auto_start(BusContext *context, DBusConnection *connection)
{
    ...
    flags = 0;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &nonexistent, DBUS_TYPE_UINT32, &flags,
                                  DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
...
static dbus_bool_t check_nonexistent_service_auto_start(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(NONEXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite",
...
static dbus_bool_t check_base_service_activated(BusContext *context, DBusConnection *connection,
                                                DBusMessage *initial_message, const char **base_service_p)
{
    DBusMessage *message;
    dbus_bool_t retval;
    DBusError error;
    ...
    reget_service_name_arg:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &base_service, DBUS_TYPE_STRING, &old_owner,
                                   DBUS_TYPE_STRING, &base_service_from_bus, DBUS_TYPE_INVALID)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                dbus_error_free(&error);
                _dbus_wait_for_memory();
        ...
        if (strcmp(base_service, base_service_from_bus) != 0) {
            _dbus_warn("Expected base service activation, got \"%s\" instead with owner \"%s\"", base_service,
...
static dbus_bool_t check_service_activated(BusContext *context, DBusConnection *connection, const char *activated_name,
                                           const char *base_service_name, DBusMessage *initial_message)
{
    DBusMessage *message;
    dbus_bool_t retval;
    DBusError error;
    ...
    reget_service_name_arg:
        ...
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &service_name, DBUS_TYPE_STRING, &old_owner,
                                   DBUS_TYPE_STRING, &base_service_from_bus, DBUS_TYPE_INVALID)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                dbus_error_free(&error);
                _dbus_wait_for_memory();
...
static dbus_bool_t check_service_auto_activated(BusContext *context, DBusConnection *connection,
                                                const char *activated_name, const char *base_service_name,
...
static dbus_bool_t check_service_deactivated(BusContext *context, DBusConnection *connection,
                                             const char *activated_name, const char *base_service)
{
    dbus_bool_t retval;
    CheckServiceOwnerChangedData socd;
...
static dbus_bool_t check_send_exit_to_service(BusContext *context, DBusConnection *connection, const char *service_name,
                                              const char *base_service)
{
    ...
    message = dbus_message_new_method_call(service_name, "/org/freedesktop/TestSuite", "org.freedesktop.TestSuite",
                                           "Exit");

    if (message == NULL) {
        /* Do this again; we still need the service to exit... */
        if (!check_send_exit_to_service(context, connection, service_name, base_service))
            goto out;
    ...
    if (got_error) {
        ...
        if (!check_service_deactivated(context, connection, service_name, base_service))
            goto out;
...
static dbus_bool_t check_got_error(BusContext *context, DBusConnection *connection, const char *first_error_name, ...)
{
    ...
    if (!error_found) {
        _dbus_warn("Expected error %s or other, got %s instead", first_error_name,
                   dbus_message_get_error_name(message));
        goto out;
    }
...
static dbus_bool_t check_existent_service_no_auto_start(BusContext *context, DBusConnection *connection)
{
   ...
   */
    ...
    if (dbus_message_get_type(message) == DBUS_MESSAGE_TYPE_ERROR) {
        ...
        if (!check_base_service_activated(context, connection, message, &base_service))
            goto out;
           ...
           */
             ...
             */
                if (message_kind != GOT_ERROR) {
                    ...
                    if (!check_got_error(context, connection, DBUS_ERROR_SPAWN_CHILD_EXITED, DBUS_ERROR_NO_MEMORY,
                                         NULL))
                        goto out;
                }
                break;
            ...
            case GOT_SERVICE_CREATED:
                ...
                if (!check_send_exit_to_service(context, connection, EXISTENT_SERVICE_NAME, base_service))
                    goto out;
...
static dbus_bool_t check_segfault_service_no_auto_start(BusContext *context, DBusConnection *connection)
{
    ...
    flags = 0;
    if (!dbus_message_append_args(message, DBUS_TYPE_STRING, &segv_service, DBUS_TYPE_UINT32, &flags,
                                  DBUS_TYPE_INVALID)) {
        dbus_message_unref(message);
        return TRUE;
    }
...
static dbus_bool_t check_existent_hello_from_self(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(EXISTENT_SERVICE_NAME, "/org/freedesktop/TestSuite",
...
static dbus_bool_t check_launch_service_file_missing(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(SERVICE_FILE_MISSING_NAME, "/org/freedesktop/TestSuite",
                                           "org.freedesktop.TestSuite", "Echo");
...
static dbus_bool_t check_launch_service_user_missing(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(SERVICE_USER_MISSING_NAME, "/org/freedesktop/TestSuite",
                                           "org.freedesktop.TestSuite", "Echo");
...
static dbus_bool_t check_launch_service_exec_missing(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(SERVICE_EXEC_MISSING_NAME, "/org/freedesktop/TestSuite",
                                           "org.freedesktop.TestSuite", "Echo");
...
static dbus_bool_t check_launch_service_service_missing(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(SERVICE_SERVICE_MISSING_NAME, "/org/freedesktop/TestSuite",
                                           "org.freedesktop.TestSuite", "Echo");
...
static dbus_bool_t check_shell_fail_service_auto_start(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(SHELL_FAIL_SERVICE_NAME, "/org/freedesktop/TestSuite",
                                           "org.freedesktop.TestSuite", "Echo");
...
static dbus_bool_t check_shell_service_success_auto_start(BusContext *context, DBusConnection *connection)
{
    ...
    message = dbus_message_new_method_call(SHELL_SUCCESS_SERVICE_NAME, "/org/freedesktop/TestSuite",
                                           "org.freedesktop.TestSuite", "Echo");
       ...
       */
        ...
        switch (message_kind) {
            case GOT_SERVICE_CREATED:
                ...
                /* Check that ServiceOwnerChanged (creation) was correctly received */
                if (!check_service_auto_activated(context, connection, SHELL_SUCCESS_SERVICE_NAME, base_service,
                                                  message))
                    goto out;
   ...
   */
    ...
    if (!dbus_message_get_args(message, NULL, DBUS_TYPE_STRING, &argv[0], DBUS_TYPE_STRING, &argv[1], DBUS_TYPE_STRING,
                               &argv[2], DBUS_TYPE_STRING, &argv[3], DBUS_TYPE_STRING, &argv[4], DBUS_TYPE_STRING,
                               &argv[5], DBUS_TYPE_STRING, &argv[6], DBUS_TYPE_INVALID)) {
   ...
   */
    ...
    if (!check_send_exit_to_service(context, connection, SHELL_SUCCESS_SERVICE_NAME, base_service))
        goto out;
...
static dbus_bool_t check_get_services(BusContext *context, DBusConnection *connection, const char *method,
                                      char ***services, int *len)
{
    DBusMessage *message;
    dbus_uint32_t serial;
    dbus_bool_t retval;
...
static void check2_try_iterations(BusContext *context, DBusConnection *connection, const char *description,
                                  Check2Func func)
{
    Check2Data d;
...
static dbus_bool_t bus_dispatch_test_conf(const DBusString *test_data_dir, const char *filename,
                                          dbus_bool_t use_launcher)
{
    ...
    check2_try_iterations(context, NULL, "create_and_hello", check_hello_connection);
...
dbus_bool_t bus_dispatch_test(const char *test_data_dir_cstr)
{
    ...
    if (!bus_dispatch_test_conf(&test_data_dir, "valid-config-files/debug-allow-all.conf", FALSE))
        return FALSE;
...
dbus_bool_t bus_dispatch_sha1_test(const char *test_data_dir_cstr)
{
    ...
    check2_try_iterations(context, NULL, "create_and_hello_sha1", check_hello_connection);
...
dbus_bool_t bus_unix_fds_passing_test(const char *test_data_dir_cstr)
{
    ...
    if (!dbus_message_append_args(m, DBUS_TYPE_UNIX_FD, one, DBUS_TYPE_UNIX_FD, two, DBUS_TYPE_UNIX_FD, two,
                                  DBUS_TYPE_INVALID))
    ...
    if (!dbus_message_get_args(m, &error, DBUS_TYPE_UNIX_FD, &x, DBUS_TYPE_UNIX_FD, &y, DBUS_TYPE_UNIX_FD, &z,
                               DBUS_TYPE_INVALID))
