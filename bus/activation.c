...
static dbus_bool_t update_desktop_file_entry(BusActivation *activation, BusServiceDirectory *s_dir,
                                             DBusString *filename, BusDesktopFile *desktop_file, DBusError *error)
{
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_NAME, &name, error))
        goto out;
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_SYSTEMD_SERVICE, &systemd_service,
                                     &tmp_error)) {
        _DBUS_ASSERT_ERROR_IS_SET(&tmp_error);
        /* if we got OOM, then exit */
        if (dbus_error_has_name(&tmp_error, DBUS_ERROR_NO_MEMORY)) {
    ...
    if (!bus_desktop_file_get_string(desktop_file, DBUS_SERVICE_SECTION, DBUS_SERVICE_ASSUMED_APPARMOR_LABEL,
    ...
    if (entry == NULL) /* New file */
    {
        ...
        if (_dbus_string_equal(&expected_name, filename)) {
            ...
            bus_context_log_and_set_error(activation->context, DBUS_SYSTEM_LOG_WARNING, error, DBUS_ERROR_FAILED,
...
static dbus_bool_t check_service_file(BusActivation *activation, BusActivationEntry *entry,
                                      BusActivationEntry **updated_entry, DBusError *error)
{
    DBusStat stat_buf;
    dbus_bool_t retval;
...
dbus_bool_t bus_activation_reload(BusActivation *activation, const DBusString *address, DBusList **directories,
                                  DBusError *error)
{
    DBusList *link;
    char *dir;
    ...
    _dbus_list_clear_full(&activation->directories, (DBusFreeFunction)bus_service_directory_unref);
...
BusActivation *bus_activation_new(BusContext *context, const DBusString *address, DBusList **directories,
                                  DBusError *error)
{
    BusActivation *activation;
...
static dbus_bool_t add_bus_environment(BusActivation *activation, DBusError *error)
{
    ...
    if (type != NULL) {
        if (!bus_activation_set_environment_variable(activation, "DBUS_STARTER_BUS_TYPE", type, error))
            return FALSE;

        if (strcmp(type, "session") == 0) {
            if (!bus_activation_set_environment_variable(activation, "DBUS_SESSION_BUS_ADDRESS",
                                                         activation->server_address, error))
                return FALSE;
        } else if (strcmp(type, "system") == 0) {
            if (!bus_activation_set_environment_variable(activation, "DBUS_SYSTEM_BUS_ADDRESS",
                                                         activation->server_address, error))
                return FALSE;
        }
    }
...
static void restore_pending(void *data)
{
    ...
    _dbus_hash_table_insert_string_preallocated(d->pending_activation->activation->pending_activations, d->hash_entry,
...
static dbus_bool_t add_restore_pending_to_transaction(BusTransaction *transaction,
                                                      BusPendingActivation *pending_activation)
{
...
dbus_bool_t bus_activation_service_created(BusActivation *activation, const char *service_name,
...
dbus_bool_t bus_activation_send_pending_auto_activation_messages(BusActivation *activation, BusService *service,
                                                                 BusTransaction *transaction)
{
    ...
    while (link != NULL) {
        ...
        if (entry->auto_activation &&
            ...
            if (!bus_dispatch_matches(transaction, entry->connection, addressed_recipient, entry->activation_message,
                                      &error)) {
               ...
               * (see fd.o #78979, CVE-2014-3477). */
                if (!bus_transaction_send_error_reply(transaction, entry->connection, &error,
...
static void handle_servicehelper_exit_error(int exit_code, DBusError *error)
{
    switch (exit_code) {
        ...
        default:
            dbus_set_error(error, DBUS_ERROR_SPAWN_CHILD_EXITED, "Launch helper exited with unknown return code %i",
                           exit_code);
            break;
    }
}
   ...
   */
    ...
    if (_dbus_babysitter_get_child_exited(babysitter)) {
       ...
       * exec error */
        ...
        if (activation_failed) {
            bus_context_log(pending_activation->activation->context, DBUS_SYSTEM_LOG_INFO,
                            "Activated service '%s' failed: %s", pending_activation->service_name, error.message);
...
static dbus_bool_t pending_activation_timed_out(void *data)
{
   ...
   */
    ...
    bus_context_log_and_set_error(context, DBUS_SYSTEM_LOG_WARNING, &error, DBUS_ERROR_TIMED_OUT,
...
static dbus_bool_t add_cancel_pending_to_transaction(BusTransaction *transaction,
                                                     BusPendingActivation *pending_activation)
{
...
static BusActivationEntry *activation_find_entry(BusActivation *activation, const char *service_name, DBusError *error)
{
    ...
    if (!entry) {
        dbus_set_error(error, DBUS_ERROR_SERVICE_UNKNOWN, "The name %s was not provided by any .service files",
                       service_name);
        return NULL;
    }
...
dbus_bool_t bus_activation_set_environment_variable(BusActivation *activation, const char *key, const char *value,
                                                    DBusError *error)
{
    char *hash_key;
    char *hash_value;
    ...
    if (!_dbus_hash_table_insert_string(activation->environment, hash_key, hash_value))
        goto out;
 ...
 */
dbus_bool_t bus_activation_activate_service(BusActivation *activation, DBusConnection *connection,
                                            ...
                                            DBusMessage *activation_message, const char *service_name, DBusError *error)
{
    ...
    if (auto_activation && entry != NULL &&
        !bus_context_check_security_policy(activation->context, transaction, connection, /* sender */
   ...
   */
    ...
    if (was_pending_activation) {
        ...
        pending_activation->timeout = _dbus_timeout_new(bus_context_get_activation_timeout(activation->context),
        ...
        if (!_dbus_hash_table_insert_string(activation->pending_activations, pending_activation->service_name,
                                            pending_activation)) {
           ...
           * systemd is not actually there to receive it yet */
            ...
            if (service != NULL) {
                bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                                ...
                                service_name, entry->systemd_service, bus_connection_get_name(connection),
                ...
                retval = bus_activation_activate_service(activation, NULL, activation_transaction, TRUE, message,
            ...
            if (!retval) {
                bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                                "Failed to activate via systemd: service name='%s' unit='%s'", service_name,
   ...
   * an early-return */
    ...
    if (servicehelper != NULL)
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                        "Activating service name='%s' requested by '%s' (%s) (using servicehelper)", service_name,
                        bus_connection_get_name(connection), bus_connection_get_loginfo(connection));
    else
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO,
                        "Activating service name='%s' requested by '%s' (%s)", service_name,
                        bus_connection_get_name(connection), bus_connection_get_loginfo(connection));
    ...
    if (!_dbus_spawn_async_with_babysitter(&pending_activation->babysitter, service_name, argv, envp, flags,
                                           child_setup, activation, &tmp_error)) {
        _dbus_verbose("Failed to spawn child\n");
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO, "Failed to activate service %s: %s", service_name,
                        tmp_error.message);
        _DBUS_ASSERT_ERROR_IS_SET(&tmp_error);
        dbus_move_error(&tmp_error, error);
        dbus_free_string_array(argv);
    ...
    _dbus_babysitter_set_result_function(pending_activation->babysitter, pending_activation_finished_cb,
                                         pending_activation);

    if (!_dbus_babysitter_set_watch_functions(pending_activation->babysitter, add_babysitter_watch,
                                              remove_babysitter_watch, toggle_babysitter_watch, pending_activation,
                                              NULL)) {
        BUS_SET_OOM(error);
    ...
    bus_context_log(
            activation->context, DBUS_SYSTEM_LOG_INFO,
            "Cannot activate service name='%s' requested by '%s' (%s): SystemdService not configured and dbus was compiled with --disable-traditional-activation",
            service_name, bus_connection_get_name(connection), bus_connection_get_loginfo(connection));
#endif
...
dbus_bool_t dbus_activation_systemd_failure(BusActivation *activation, DBusMessage *message)
{
    ...
    if (dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &unit, DBUS_TYPE_STRING, &code, DBUS_TYPE_STRING, &str,
                              DBUS_TYPE_INVALID))
    ...
    if (unit) {
        ...
        bus_context_log(activation->context, DBUS_SYSTEM_LOG_INFO, "Activation via systemd failed for unit '%s': %s",
                        unit, str);
