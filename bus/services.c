...
struct BusRegistry {
    int refcount;
    // 引用计数，用于跟踪 BusRegistry 对象的引用次数。
    // 每当有新的引用指向 BusRegistry 对象时，refcount 增加；
    // 每当引用释放时，refcount 减少。当 refcount 变为 0 时，表示对象可以被安全地销毁。

    BusContext *context;
    // 指向与此注册表关联的 BusContext 对象的指针。
    // 该上下文提供了访问 D-Bus 总线环境和配置的机制。
    ...
    // 使用内存池可以减少频繁的内存分配和释放，提高性能。

    DBusMemPool *owner_pool;
    // 内存池，用于高效地分配和管理 BusOwner 结构体的内存。
    // 与 service_pool 类似，使用内存池可以提高内存管理的效率。

    DBusHashTable *service_sid_table;
    // 哈希表，用于存储服务的安全标识符（SID）信息。
    // 该字段在一些系统中可能用于存储服务的安全相关信息，如 Windows 系统中的 SID。
};
...
BusService *bus_registry_ensure(BusRegistry *registry, const DBusString *service_name,
                                DBusConnection *owner_connection_if_created, dbus_uint32_t flags,
                                BusTransaction *transaction, DBusError *error)
{
    ...
    if (!bus_activation_service_created(bus_context_get_activation(registry->context), service->name, transaction,
                                        error)) {
        bus_service_unref(service);
        return NULL;
    }
...
dbus_bool_t bus_registry_acquire_service(BusRegistry *registry, DBusConnection *connection,
                                         ...
                                         BusTransaction *transaction, DBusError *error)
{
    ...
    if (!_dbus_validate_bus_name(service_name, 0, _dbus_string_get_length(service_name))) {
        dbus_set_error(error, DBUS_ERROR_INVALID_ARGS, "Requested bus name \"%s\" is not valid",
    ...
    if (_dbus_string_get_byte(service_name, 0) == ':') {
        ...
        dbus_set_error(error, DBUS_ERROR_INVALID_ARGS, "Cannot acquire a service starting with ':' such as \"%s\"",
                       _dbus_string_get_const_data(service_name));
   ...
   */
    ...
    if (!bus_apparmor_allows_acquire_service(connection, bus_context_get_type(registry->context),
    ...
    if (old_owner_conn == NULL) {
    ...
    } else if (((flags & DBUS_NAME_FLAG_DO_NOT_QUEUE) && !(bus_service_get_allow_replacement(service))) ||
        ...
        *result = DBUS_REQUEST_NAME_REPLY_EXISTS;
        ...
        if (!bus_service_add_owner(service, connection, flags, transaction, error))
            goto out;
       ...
       */
        ...
        if (primary_owner->do_not_queue) {
            if (!bus_service_remove_owner(service, old_owner_conn, transaction, error))
                goto out;
        } else {
            if (!bus_service_swap_owner(service, old_owner_conn, transaction, error))
                goto out;
        }
...
dbus_bool_t bus_registry_release_service(BusRegistry *registry, DBusConnection *connection,
                                         const DBusString *service_name, dbus_uint32_t *result,
                                         BusTransaction *transaction, DBusError *error)
{
    dbus_bool_t retval;
    BusService *service;
    ...
    if (!_dbus_validate_bus_name(service_name, 0, _dbus_string_get_length(service_name))) {
        dbus_set_error(error, DBUS_ERROR_INVALID_ARGS, "Given bus name \"%s\" is not valid",
                       _dbus_string_get_const_data(service_name));
    ...
    if (_dbus_string_equal_c_str(service_name, DBUS_SERVICE_DBUS)) {
        /* Not allowed; the base service name cannot be created or released */
        dbus_set_error(error, DBUS_ERROR_INVALID_ARGS, "Cannot release the %s service because it is owned by the bus",
                       DBUS_SERVICE_DBUS);
...
static dbus_bool_t add_cancel_ownership_to_transaction(BusTransaction *transaction, BusService *service,
                                                       BusOwner *owner)
{
...
dbus_bool_t bus_service_add_owner(BusService *service, DBusConnection *connection, dbus_uint32_t flags,
                                  BusTransaction *transaction, DBusError *error)
{
    BusOwner *bus_owner;
...
static void free_ownership_restore_data(void *data)
{
    ...
    if (d->hash_entry)
        _dbus_hash_table_free_preallocated_entry(d->service->registry->service_hash, d->hash_entry);
...
static dbus_bool_t add_restore_ownership_to_transaction(BusTransaction *transaction, BusService *service,
                                                        BusOwner *owner)
{
...
dbus_bool_t bus_service_swap_owner(BusService *service, DBusConnection *connection, BusTransaction *transaction,
                                   DBusError *error)
{
    DBusList *swap_link;
    BusOwner *primary_owner;
   ...
   */
    ...
    if (!bus_driver_send_service_lost(connection, service->name, transaction, error))
        return FALSE;
    ...
    if (service->owners == NULL) {
        ...
        if (!bus_driver_send_service_owner_changed(service->name, bus_connection_get_name(connection),
...
/* this function is self-cancelling if you cancel the transaction */
dbus_bool_t bus_service_remove_owner(BusService *service, DBusConnection *connection, BusTransaction *transaction,
                                     DBusError *error)
{
    BusOwner *primary_owner;
   ...
   */
    ...
    if (service->owners == NULL) {
        ...
        if (!bus_driver_send_service_owner_changed(service->name, bus_connection_get_name(connection), NULL,
                                                   transaction, error))
            return FALSE;
    } else {
        DBusList *link;
...
dbus_bool_t bus_service_owner_in_queue(BusService *service, DBusConnection *connection)
{
    DBusList *link;
...
dbus_bool_t bus_service_list_queued_owners(BusService *service, DBusList **return_list)
{
    DBusList *link;
