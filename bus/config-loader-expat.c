...
static void expat_StartElementHandler(void *userData, const XML_Char *name, const XML_Char **atts)
{
   ...
   */
    ...
    if (!bus_config_parser_start_element(context->parser, name, (const char **)names, (const char **)values,
                                         context->error)) {
        dbus_free(names);
        dbus_free(values);
        context->failed = TRUE;
...
BusConfigParser *bus_config_load(const DBusString *file, dbus_bool_t is_toplevel, const BusConfigParser *parent,
                                 DBusError *error)
{
    XML_Parser expat;
    const char *filename;
                ...
                if (e == XML_ERROR_NO_MEMORY)
                    ...
                    dbus_set_error(error, DBUS_ERROR_FAILED, "Error in file %s, line %lu, column %lu: %s\n", filename,
                               ...
                               * cope with. */
                                   ...
                                   (unsigned long)XML_GetCurrentColumnNumber(expat), XML_ErrorString(e));
            }
