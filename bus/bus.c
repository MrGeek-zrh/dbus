 ...
 *
 * @param context 指向 BusContext 的指针，表示 DBus 守护进程的上下文。这个context 是从server 的 new_connection_handler_datr
 ...
 */
static dbus_bool_t process_config_every_time(BusContext *context, BusConfigParser *parser, dbus_bool_t is_reload,
                                             DBusError *error)
{
                            ...
                            DBusError *error)
{
    ...
    // 确保 FORK_NEVER 和 FORK_ALWAYS 标志不会同时设置
    ...
    // 分配一个数据槽用于存储服务器数据
    // 在 D-Bus 中，数据槽机制允许在对象（如 DBusConnection、DBusServer 等）上附加任意数据。每个数据槽由一个唯一的整数 ID 标识，通过这个 ID 可以在对象上存取对应的用户数据。这个机制提供了一种灵活的方法来扩展对象，而不需要修改它们的定义。
    ...
    // 生成一个 UUID 用于标识这个 BusContext
    if (!_dbus_generate_uuid(&context->uuid, error))
        goto failed;

    // 复制配置文件路径到 BusContext
    ...
    // 如果提供了 print_addr_pipe,则将 D-Bus 地址写入该管道
...
#ifdef DBUS_WIN
    // 在 Windows 上,如果提供了 ready_event_handle,则通知已准备好接收连接
    ...
    // 根据配置,决定是否需要进行 fork 操作
    {
        ...
        if (((flags & BUS_CONTEXT_FLAG_FORK_NEVER) == 0 && context->fork) || (flags & BUS_CONTEXT_FLAG_FORK_ALWAYS)) {
            ...
            // 如果不需要 fork,则直接写入 PID 文件和管道
            if (!_dbus_write_pid_to_file_and_pipe(context->pidfile ? &u : NULL, print_pid_pipe, _dbus_getpid(),
                                                  error)) {
                _DBUS_ASSERT_ERROR_IS_SET(error);
                goto failed;
            }
    ...
    // 打印 SELinux 和 AppArmor 的启用状态
    if (bus_selinux_enabled()) {
        if (context->syslog)
            bus_context_log(context, DBUS_SYSTEM_LOG_INFO, "SELinux support is enabled\n");
    }

    if (bus_apparmor_enabled()) {
        if (context->syslog)
            bus_context_log(context, DBUS_SYSTEM_LOG_INFO, "AppArmor D-Bus mediation is enabled\n");
    }

    // 处理配置文件中的 <associate/> 元素
 ...
 */
static dbus_bool_t bus_context_send_activatable_services_changed(BusContext *context, DBusError *error)
{
    DBusMessage *message;
    ...
    message = dbus_message_new_signal(DBUS_PATH_DBUS, DBUS_INTERFACE_DBUS, "ActivatableServicesChanged");
...

    {
        ...
        if (!bus_context_send_activatable_services_changed(context, &local_error))
            bus_context_log(context, DBUS_SYSTEM_LOG_INFO,
                            "Unable to send signal that configuration has been reloaded: %s", local_error.message);
...
BusClientPolicy *bus_context_create_client_policy(BusContext *context, DBusConnection *connection,
                                                  BusClientPolicy *previous, DBusError *error)
{
    ...
    client = bus_policy_create_client_policy(context->policy, connection, &local_error);
...
void bus_context_log_and_set_error(BusContext *context, DBusSystemLogSeverity severity, DBusError *error,
 ...
 */
static void complain_about_message(BusContext *context, const char *error_name, const char *complaint,
                                   int matched_rules, DBusMessage *message, DBusConnection *sender,
                                   DBusConnection *proposed_recipient, dbus_bool_t requested_reply, dbus_bool_t log,
                                   DBusError *error)
{
    DBusError stack_error = DBUS_ERROR_INIT;
    ...
    dbus_set_error(&stack_error, error_name,
                   ...
                   complaint, matched_rules, dbus_message_type_to_string(dbus_message_get_type(message)), sender_name,
                   sender_loginfo, nonnull(dbus_message_get_interface(message), "(unset)"),
                   nonnull(dbus_message_get_member(message), "(unset)"),
                   nonnull(dbus_message_get_error_name(message), "(unset)"), requested_reply,
                   nonnull(dbus_message_get_destination(message), DBUS_SERVICE_DBUS), proposed_recipient_loginfo);
 ...
 */
dbus_bool_t bus_context_check_security_policy(BusContext *context, BusTransaction *transaction, DBusConnection *sender,
                                              ...
                                              DBusError *error)
{
    ...
    _dbus_assert(dest != NULL || type == DBUS_MESSAGE_TYPE_SIGNAL ||
                 (sender == NULL && !bus_connection_is_active(proposed_recipient)));
    _dbus_assert(type == DBUS_MESSAGE_TYPE_SIGNAL || addressed_recipient != NULL || activation_entry != NULL ||
    ...
    if (sender != NULL) {
           ...
           */
            if (dbus_message_get_reply_serial(message) != 0) {
                if (proposed_recipient != NULL /* not to the bus driver */ &&
                    ...
                    requested_reply = bus_connections_check_reply(bus_connection_get_connections(sender), transaction,
       ...
       */
        if (!bus_selinux_allows_send(
                    sender, proposed_recipient, dbus_message_type_to_string(dbus_message_get_type(message)),
                    dbus_message_get_interface(message), dbus_message_get_member(message),
                    dbus_message_get_error_name(message), dest ? dest : DBUS_SERVICE_DBUS, activation_entry, error)) {
            if (error != NULL && !dbus_error_is_set(error)) {
       ...
       * go on with the standard checks.
       */
        if (!bus_apparmor_allows_send(sender, proposed_recipient, requested_reply, bus_context_get_type(context),
                                      dbus_message_get_type(message), dbus_message_get_path(message),
                                      dbus_message_get_interface(message), dbus_message_get_member(message),
                                      dbus_message_get_error_name(message), dest ? dest : DBUS_SERVICE_DBUS,
                                      src ? src : DBUS_SERVICE_DBUS, activation_entry, error))
            return FALSE;
           ...
           */
            if (proposed_recipient == NULL && dbus_message_is_method_call(message, DBUS_INTERFACE_DBUS, "Hello")) {
                ...
                dbus_set_error(error, DBUS_ERROR_ACCESS_DENIED,
                               "Client tried to send a message other than %s without being registered", "Hello");
    ...
    log = FALSE;
    if (sender_policy && !bus_client_policy_check_can_send(sender_policy, context->registry, requested_reply,
                                                           proposed_recipient, message, &toggles, &log)) {
        complain_about_message(context, DBUS_ERROR_ACCESS_DENIED, "Rejected send message", toggles, message, sender,
                               proposed_recipient, requested_reply, (addressed_recipient == proposed_recipient), error);
    ...
    if (log) {
       ...
       * compatibility. */
        complain_about_message(context, DBUS_ERROR_ACCESS_DENIED, "Would reject message", toggles, message, sender,
                               proposed_recipient, requested_reply, TRUE, NULL);
    }

    if (recipient_policy &&
        !bus_client_policy_check_can_receive(recipient_policy, context->registry, requested_reply, sender,
                                             addressed_recipient, proposed_recipient, message, &toggles)) {
        complain_about_message(context, DBUS_ERROR_ACCESS_DENIED, "Rejected receive message", toggles, message, sender,
                               proposed_recipient, requested_reply, (addressed_recipient == proposed_recipient), error);
    ...
    if (proposed_recipient &&
        ...
        complain_about_message(context, DBUS_ERROR_LIMITS_EXCEEDED, "Rejected: destination has a full message queue", 0,
                               message, sender, proposed_recipient, requested_reply, TRUE, error);
   ...
   */
    if (type == DBUS_MESSAGE_TYPE_METHOD_CALL && sender && addressed_recipient &&
        addressed_recipient == proposed_recipient && /* not eavesdropping */
        !bus_connections_expect_reply(bus_connection_get_connections(sender), transaction, sender, addressed_recipient,
                                      message, error)) {
