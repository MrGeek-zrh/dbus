...
static dbus_bool_t bus_container_instance_emit_removed(BusContainerInstance *self)
{
    ...
    if (message == NULL || !dbus_message_set_sender(message, DBUS_SERVICE_DBUS) ||
        !dbus_message_append_args(message, DBUS_TYPE_OBJECT_PATH, &self->path, DBUS_TYPE_INVALID) ||
    ...
    if (--self->refcount == 0) {
       ...
       * because the paths are globally unique. */
        if (self->path != NULL && self->containers->instances_by_path != NULL &&
            _dbus_hash_table_remove_string(self->containers->instances_by_path, self->path)) {
            DBusHashIter entry;
            uintptr_t n = 0;

            if (!_dbus_hash_iter_lookup(self->containers->n_containers_by_user, (void *)(uintptr_t)self->uid, FALSE,
                                        &entry))
...
static BusContainerInstance *bus_container_instance_new(BusContext *context, BusContainers *containers,
                                                        DBusConnection *creator, DBusError *error)
{
   ...
   * only non-C99 platform we support is MSVC on Windows. */
    if (!_dbus_string_append_printf(&path, "/org/freedesktop/DBus/Containers1/c%" PRIu64,
...
static void new_connection_cb(DBusServer *server, DBusConnection *new_connection, void *data)
{
    ...
    if (!dbus_connection_set_data(new_connection, contained_data_slot, bus_container_instance_ref(instance),
   ...
   * lowest-common-denominator implementation. */
    dbus_connection_set_unix_user_function(new_connection, allow_same_uid_only,
                                           /* The static assertion above
                                           * allow_same_uid_only ensures that
                                           * this cast does not lose
                                           * information */
                                           (void *)(uintptr_t)instance->uid, NULL);
}
...
static dbus_bool_t bus_container_instance_listen(BusContainerInstance *self, DBusError *error)
{
    ...
    dbus_server_set_new_connection_function(self->server, new_connection_cb, bus_container_instance_ref(self),
                                            (DBusFreeFunction)bus_container_instance_unref);
    return TRUE;
}

dbus_bool_t bus_containers_handle_add_server(DBusConnection *connection, BusTransaction *transaction,
                                             DBusMessage *message, DBusError *error)
{
    ...
    if (creator_data == NULL) {
        ...
        if (!dbus_connection_set_data(connection, container_creator_data_slot, creator_data,
   ...
   * int value. */
    ...
    if (metadata_size > limit) {
        ...
        dbus_set_error(&local_error, DBUS_ERROR_LIMITS_EXCEEDED,
                       ...
                       bus_connection_get_name(connection), bus_connection_get_loginfo(connection), metadata_size,
                       limit);
    ...
    if (!_dbus_hash_iter_lookup(containers->n_containers_by_user,
                                ...
                                * uintptr_t, so this can't lose information */
                                (void *)(uintptr_t)instance->uid, TRUE, &n_containers_by_user_entry))
        goto oom;
   ...
   * unsigned. */
    if (limit <= 0 || this_user_containers >= (unsigned)limit) {
        ...
        dbus_set_error(&local_error, DBUS_ERROR_LIMITS_EXCEEDED,
                       ...
                       bus_connection_get_name(connection), bus_connection_get_loginfo(connection), instance->uid,
                       limit);
        bus_context_log_literal(context, DBUS_SYSTEM_LOG_WARNING, local_error.message);
        dbus_move_error(&local_error, error);
        goto fail;
    ...
    if (!_dbus_hash_table_insert_string(containers->instances_by_path, instance->path, instance))
        goto oom;
   ...
   * creating our own server, and defer listening on it until later */
    ...
    if (!dbus_message_iter_open_container(&writer, DBUS_TYPE_ARRAY, DBUS_TYPE_BYTE_AS_STRING, &array_writer))
        goto oom;
...
dbus_bool_t bus_containers_supported_arguments_getter(BusContext *context, DBusMessageIter *var_iter)
{
    ...
    return dbus_message_iter_open_container(var_iter, DBUS_TYPE_ARRAY, DBUS_TYPE_STRING_AS_STRING, &arr_iter) &&
           dbus_message_iter_close_container(var_iter, &arr_iter);
}

dbus_bool_t bus_containers_handle_stop_instance(DBusConnection *connection, BusTransaction *transaction,
                                                DBusMessage *message, DBusError *error)
{
    BusContext *context;
    BusContainers *containers;
    ...
    if (containers->instances_by_path != NULL) {
        instance = _dbus_hash_table_lookup_string(containers->instances_by_path, path);
    }
    ...
    for (iter = _dbus_list_get_first_link(&instance->connections); iter != NULL;
...
dbus_bool_t bus_containers_handle_stop_listening(DBusConnection *connection, BusTransaction *transaction,
                                                 DBusMessage *message, DBusError *error)
{
    BusContext *context;
    BusContainers *containers;
 ...
 */
static BusContainerInstance *connection_get_instance(DBusConnection *connection)
{
    if (connection == NULL)
        return NULL;
...
dbus_bool_t bus_containers_handle_get_connection_instance(DBusConnection *caller, BusTransaction *transaction,
                                                          DBusMessage *message, DBusError *error)
{
    ...
    if (!dbus_message_iter_open_container(&writer, DBUS_TYPE_ARRAY, "{sv}", &arr_writer))
        goto oom;

    if (!bus_driver_fill_connection_credentials(NULL, instance->creator, &arr_writer)) {
    ...
    if (!dbus_message_append_args(reply, DBUS_TYPE_STRING, &instance->type, DBUS_TYPE_STRING, &instance->name,
                                  DBUS_TYPE_INVALID))
        goto oom;
...
dbus_bool_t bus_containers_handle_get_instance_info(DBusConnection *connection, BusTransaction *transaction,
                                                    DBusMessage *message, DBusError *error)
{
    BusContext *context;
    BusContainers *containers;
...
dbus_bool_t bus_containers_handle_request_header(DBusConnection *caller, BusTransaction *transaction,
                                                 DBusMessage *message, DBusError *error)
{
    DBusMessage *reply = NULL;
   ...
   * because otherwise it isn't transactional */
    if (reply == NULL || !bus_transaction_send_from_driver(transaction, caller, reply)) {
        BUS_SET_OOM(error);
        goto out;
    }

    bus_connection_request_headers(caller, BUS_EXTRA_HEADERS_CONTAINER_INSTANCE);
    ret = TRUE;
...
dbus_bool_t bus_containers_connection_is_contained(DBusConnection *connection, const char **path, const char **type,
                                                   const char **name)
{
#ifdef DBUS_ENABLE_CONTAINERS
    BusContainerInstance *instance;
