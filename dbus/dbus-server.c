...
static dbus_bool_t protected_change_watch(DBusServer *server, DBusWatch *watch, DBusWatchAddFunction add_function,
...
static dbus_bool_t protected_change_timeout(DBusServer *server, DBusTimeout *timeout,
    ...
    for (i = 0; i < len; i++) {
        ...
        for (j = 0; j < (int)_DBUS_N_ELEMENTS(listen_funcs); ++j) {
            ...
            if (result == DBUS_SERVER_LISTEN_OK) {
                ...
                _dbus_assert(server == NULL);
                _DBUS_ASSERT_ERROR_IS_CLEAR(&tmp_error);
                dbus_set_error(error, DBUS_ERROR_ADDRESS_IN_USE, "Address '%s' already used",
...
out:
    ...
    if (!handled_once) {
        _DBUS_ASSERT_ERROR_IS_CLEAR(error);
        if (len > 0)
            dbus_set_error(error, DBUS_ERROR_BAD_ADDRESS, "Unknown address type '%s'",
                           dbus_address_entry_get_method(entries[0]));
        else
 ...
 */
void dbus_server_set_new_connection_function(DBusServer *server, DBusNewConnectionFunction function, void *data,
 ...
 * @returns #FALSE on failure (no memory)
 */
dbus_bool_t dbus_server_set_timeout_functions(DBusServer *server, DBusAddTimeoutFunction add_function,
                                              ...
                                              DBusFreeFunction free_data_function)
{
    ...
    if (timeouts) {
        SERVER_UNLOCK(server);
        result = _dbus_timeout_list_set_functions(timeouts, add_function, remove_function, toggled_function, data,
                                                  free_data_function);
        SERVER_LOCK(server);
    } else {
        _dbus_warn_check_failed("Re-entrant call to %s", _DBUS_FUNCTION_NAME);
...
dbus_bool_t dbus_server_set_data(DBusServer *server, int slot, void *data, DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &server->slot_list, slot, data, free_data_func, &old_free_func,
                                      &old_data);
