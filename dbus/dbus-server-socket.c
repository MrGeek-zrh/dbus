 ...
 *
 */
DBusServer *_dbus_server_new_for_socket(DBusSocket *fds, int n_fds, const DBusString *address, DBusNonceFile *noncefile,
                                        DBusError *error)
{
    ...
    for (i = 0; i < n_fds; i++) {
        ...
        watch = _dbus_watch_new(_dbus_socket_get_pollable(fds[i]), DBUS_WATCH_READABLE, TRUE, socket_handle_watch,
                                socket_server, NULL);
        if (watch == NULL)
            goto failed;
    ...
    if (!_dbus_server_init_base(&socket_server->base, &socket_vtable, address, error))
        goto failed;
 ...
 */
DBusServer *_dbus_server_new_for_tcp_socket(const char *host, const char *bind, const char *port, const char *family,
                                            DBusError *error, dbus_bool_t use_nonce)
{
    ...
    nlisten_fds = _dbus_listen_tcp_socket(bind, port, family, &port_str, &family_used, &listen_fds, error);
    if (nlisten_fds <= 0) {
    ...
    if (family_used && (!_dbus_string_append(&address, ",family=") || !_dbus_string_append(&address, family_used))) {
        dbus_set_error(error, DBUS_ERROR_NO_MEMORY, NULL);
        goto failed;
    }
...
DBusServerListenResult _dbus_server_listen_socket(DBusAddressEntry *entry, DBusServer **server_p, DBusError *error)
{
    ...
    *server_p = NULL;
    ...
    if (strcmp(method, "tcp") == 0 || strcmp(method, "nonce-tcp") == 0) {
        ...
        *server_p = _dbus_server_new_for_tcp_socket(host, bind, port, family, error,
