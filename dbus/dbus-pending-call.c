...
static void _dbus_pending_call_trace_ref(DBusPendingCall *pending_call, int old_refcount, int new_refcount,
                                         const char *why)
{
#ifdef DBUS_ENABLE_VERBOSE_MODE
    static int enabled = -1;

    _dbus_trace_ref("DBusPendingCall", pending_call, old_refcount, new_refcount, why, "DBUS_PENDING_CALL_TRACE",
                    &enabled);
#endif
}
 ...
 */
DBusPendingCall *_dbus_pending_call_new_unlocked(DBusConnection *connection, int timeout_milliseconds,
 ...
 void _dbus_pending_call_finish_completion(DBusPendingCall *pending)
{
    ...
    // 这个看起啦就是调用客户端设置的用来处理接收到的消息的回掉函数
    // 感觉不对
    if (pending->function) {
        void *user_data;
 ...
 * @return #FALSE on OOM
 */
dbus_bool_t _dbus_pending_call_set_timeout_error_unlocked(DBusPendingCall *pending, DBusMessage *message,
 ...
 */
dbus_bool_t _dbus_pending_call_set_data_unlocked(DBusPendingCall *pending, dbus_int32_t slot, void *data,
                                                 DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &pending->slot_list, slot, data, free_data_func, &old_free_func,
                                      &old_data);
                                         ...
                                         void *user_data, DBusFreeFunction free_user_data)
{
    ...
    if (!_dbus_pending_call_set_data_unlocked(pending, notify_user_data_slot, user_data, free_user_data))
        goto out;
 ...
 * @returns #TRUE if there was enough memory to store the data
 */
dbus_bool_t dbus_pending_call_set_data(DBusPendingCall *pending, dbus_int32_t slot, void *data,
                                       DBusFreeFunction free_data_func)
{
    dbus_bool_t retval;
