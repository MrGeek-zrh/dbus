...
int _dbus_write_socket_with_unix_fds(DBusSocket fd, const DBusString *buffer, int start, int len, const int *fds,
                                     int n_fds)
{
#ifndef HAVE_UNIX_FD_PASSING
...
int _dbus_write_socket_with_unix_fds_two(DBusSocket fd, const DBusString *buffer1, int start1, int len1,
                                         const DBusString *buffer2, int start2, int len2, const int *fds, int n_fds)
{
#ifndef HAVE_UNIX_FD_PASSING
 ...
 */
int _dbus_write_socket_two(DBusSocket fd, const DBusString *buffer1, int start1, int len1, const DBusString *buffer2,
                           int start2, int len2)
{
#if HAVE_DECL_MSG_NOSIGNAL
 ...
 * @returns total bytes written from both buffers, or -1 on error
 */
int _dbus_write_two(int fd, const DBusString *buffer1, int start1, int len1, const DBusString *buffer2, int start2,
                    int len2)
{
    _dbus_assert(buffer1 != NULL);
    _dbus_assert(start1 >= 0);
...
int _dbus_connect_unix_socket(const char *path, dbus_bool_t abstract, DBusError *error)
{
    ...
    if (connect(fd, (struct sockaddr *)&addr, _DBUS_STRUCT_OFFSET(struct sockaddr_un, sun_path) + path_len) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to connect to socket %s: %s", path,
...
int _dbus_connect_exec(const char *path, char *const argv[], DBusError *error)
{
   ...
   * parent and the child */
    ...
    pid = fork();
    if (pid < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to fork() to call %s: %s", path,
                       _dbus_strerror(errno));
        close(fds[0]);
        close(fds[1]);
        return -1;
...
int _dbus_listen_unix_socket(const char *path, dbus_bool_t abstract, DBusError *error)
{
    ...
    if (bind(listen_fd, (struct sockaddr *)&addr, _DBUS_STRUCT_OFFSET(struct sockaddr_un, sun_path) + path_len) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to bind socket \"%s\": %s", path,
                       _dbus_strerror(errno));
        _dbus_close(listen_fd, NULL);
        return -1;
    }
    ...
    if (listen(listen_fd, SOMAXCONN /* backlog */) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to listen on socket \"%s\": %s", path,
                       _dbus_strerror(errno));
        _dbus_close(listen_fd, NULL);
        return -1;
    }
...
int _dbus_listen_systemd_sockets(DBusSocket **fds, DBusError *error)
{
    ...
    for (fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + n; fd++) {
        ...
        if (r < 0) {
            dbus_set_error(error, _dbus_error_from_errno(-r), "Failed to verify systemd socket type: %s",
                           _dbus_strerror(-r));
            return -1;
        }
...
DBusSocket _dbus_connect_tcp_socket_with_nonce(const char *host, const char *port, const char *family,
                                               const char *noncefile, DBusError *error)
{
    int saved_errno = 0;
    ...
    if ((res = getaddrinfo(host, port, &hints, &ai)) != 0) {
        dbus_set_error(error, _dbus_error_from_gai(res, errno), "Failed to lookup host/port: \"%s:%s\": %s (%d)", host,
    ...
    while (tmp) {
        ...
        if (connect(fd.fd, (struct sockaddr *)tmp->ai_addr, tmp->ai_addrlen) < 0) {
            ...
            _dbus_set_error_with_inet_sockaddr(connect_error, tmp->ai_addr, tmp->ai_addrlen,
    ...
    if (!_dbus_socket_is_valid(fd)) {
        _dbus_combine_tcp_errors(&connect_errors, "Failed to connect", host, port, error);
        goto out;
    }
 ...
 */
int _dbus_listen_tcp_socket(const char *host, const char *port, const char *family, DBusString *retport,
                            const char **retfamily, DBusSocket **fds_p, DBusError *error)
{
    int saved_errno;
...
redo_lookup_with_port:
    ...
    if ((res = getaddrinfo(host, port, &hints, &ai)) != 0 || !ai) {
        dbus_set_error(error, _dbus_error_from_gai(res, errno), "Failed to lookup host/port: \"%s:%s\": %s (%d)",
    ...
    while (tmp) {
        ...
        if (setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &tcp_nodelay_on, sizeof(tcp_nodelay_on)) == -1) {
            _dbus_warn("Failed to set TCP_NODELAY socket option \"%s:%s\": %s", host ? host : "*", port,
                       _dbus_strerror(errno));
        }
        ...
        if (bind(fd, (struct sockaddr *)tmp->ai_addr, tmp->ai_addrlen) < 0) {
           ...
           */
            ...
            _dbus_set_error_with_inet_sockaddr(bind_error, tmp->ai_addr, tmp->ai_addrlen, "Failed to bind socket",
                                               saved_errno);
        ...
        if (listen(fd, 30 /* backlog */) < 0) {
            saved_errno = errno;
            _dbus_close(fd, NULL);
            _dbus_set_error_with_inet_sockaddr(error, tmp->ai_addr, tmp->ai_addrlen, "Failed to listen on socket",
                                               saved_errno);
            goto failed;
        }
            ...
            if (!port || !strcmp(port, "0")) {
                ...
                if (result == -1) {
                    ...
                    dbus_set_error(error, _dbus_error_from_errno(saved_errno),
                                   "Failed to retrieve socket name for \"%s:%s\": %s", host ? host : "*", port,
                                   _dbus_strerror(saved_errno));
                    goto failed;
                }

                if ((res = getnameinfo((struct sockaddr *)&addr, addrlen, NULL, 0, portbuf, sizeof(portbuf),
                                       NI_NUMERICHOST | NI_NUMERICSERV)) != 0) {
                    saved_errno = errno;
                    dbus_set_error(error, _dbus_error_from_gai(res, saved_errno),
                                   "Failed to resolve port \"%s:%s\": %s (%d)", host ? host : "*", port,
...
again:
   ...
   * only allows that on AF_UNIX. Try just doing a send() instead. */
    ...
    if (bytes_written < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to write credentials byte: %s",
                       _dbus_strerror(errno));
        return FALSE;
...
static dbus_bool_t add_groups_to_credentials(int client_fd, DBusCredentials *credentials, dbus_gid_t primary)
{
...
static dbus_bool_t add_linux_security_label_to_credentials(int client_fd, DBusCredentials *credentials)
{
    ...
    if (!_dbus_string_init_preallocated(&buf, len) || !_dbus_string_set_length(&buf, len))
        return FALSE;
...
again:
    ...
    if (bytes_read < 0) {
       ...
       */
        ...
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to read credentials byte: %s",
                       _dbus_strerror(errno));
        return FALSE;
...
dbus_bool_t _dbus_check_dir_is_private_to_user(DBusString *dir, DBusError *error)
{
    ...
    if (sb.st_uid != geteuid()) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "%s directory is owned by user %lu, not %lu", directory,
 ...
 */
// 获取单调时间（monotonic time），它不同于系统时间，不会因为系统时间的调整（例如手动调整系统时钟）而改变。这个时间通常用于计算时间间隔和测量时间差。
...
dbus_bool_t _dbus_concat_dir_and_file(DBusString *dir, const DBusString *next_component)
{
    ...
    if (_dbus_string_get_length(dir) == 0 || _dbus_string_get_length(next_component) == 0)
        return TRUE;
...
dbus_bool_t _dbus_generate_random_bytes(DBusString *str, int n_bytes, DBusError *error)
{
    ...
    if (result != n_bytes) {
        if (result < 0)
            dbus_set_error(error, _dbus_error_from_errno(errno), "Could not read /dev/urandom: %s",
                           _dbus_strerror(errno));
        else
...
static dbus_bool_t _dbus_set_fd_nonblocking(int fd, DBusError *error)
{
    ...
    if (val < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to get flags from file descriptor %d: %s", fd,
                       _dbus_strerror(errno));
    ...
    if (fcntl(fd, F_SETFL, val | O_NONBLOCK) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to set nonblocking flag of file descriptor %d: %s",
                       fd, _dbus_strerror(errno));
...
dbus_bool_t _dbus_socketpair(DBusSocket *fd1, DBusSocket *fd2, dbus_bool_t blocking, DBusError *error)
{
    ...
    // 尝试创建带有 SOCK_CLOEXEC 标志的 socket pair
    retval = socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, fds);
    cloexec_done = retval >= 0;

    // 如果创建失败且错误是 EINVAL 或 EPROTOTYPE，再次尝试不带 SOCK_CLOEXEC 标志
    if (retval < 0 && (errno == EINVAL || errno == EPROTOTYPE))
#endif
    {
        retval = socketpair(AF_UNIX, SOCK_STREAM, 0, fds);
    ...
    // 如果创建 socket pair 失败，设置错误并返回 FALSE
    if (retval < 0) {
    ...
    return TRUE;
#else
    // 如果没有实现 socketpair，设置错误并返回 FALSE
 ...
 */
static dbus_bool_t _read_subprocess_line_argv(const char *progpath, dbus_bool_t path_fallback, const char *const *argv,
                                              DBusString *result, DBusError *error)
{
...
#define WRITE_END 1
    if (pipe(result_pipe) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to create a pipe to call %s: %s", progpath,
                       _dbus_strerror(errno));
   ...
   * parent and the child */
    ...
    pid = fork();
    if (pid < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to fork() to call %s: %s", progpath,
                       _dbus_strerror(errno));
    ...
    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
        ...
        if (_dbus_string_get_length(&error_message) > 0)
            dbus_set_error(error, DBUS_ERROR_SPAWN_EXEC_FAILED, "%s terminated abnormally with the following error: %s",
                           progpath, _dbus_string_get_data(&error_message));
        else
            dbus_set_error(error, DBUS_ERROR_SPAWN_EXEC_FAILED, "%s terminated abnormally without any error message",
                           progpath);
        goto out;
    }
...
static void close_ignore_error(int fd)
{
    close(fd);
}
 ...
 */
static void act_on_fds_3_and_up(void (*func)(int fd))
{
    int maxfds, i;
...
dbus_bool_t _dbus_append_address_from_socket(DBusSocket fd, DBusString *address, DBusError *error)
{
    ...
    switch (socket.sa.sa_family) {
        ...
        case AF_INET6:
            ...
            if (_dbus_inet_sockaddr_to_string(&socket, size, hostip, sizeof(hostip), &family_name, &port, error)) {
                if (_dbus_string_append_printf(address, "tcp:family=%s,port=%u,host=", family_name, port) &&
                    _dbus_address_append_escaped(address, &path_str)) {
                    return TRUE;
                } else {
                    _DBUS_SET_OOM(error);
...
err:
    dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to read address from socket: %s",
                   _dbus_strerror(errno));
    return FALSE;
}
