...
static dbus_bool_t handle_server_state_waiting_for_auth(DBusAuth *auth, DBusAuthCommand command,
                                                        const DBusString *args);
static dbus_bool_t handle_server_state_waiting_for_data(DBusAuth *auth, DBusAuthCommand command,
                                                        const DBusString *args);
static dbus_bool_t handle_server_state_waiting_for_begin(DBusAuth *auth, DBusAuthCommand command,
                                                         const DBusString *args);

static const DBusAuthStateData server_state_waiting_for_auth = { "WaitingForAuth",
                                                                 handle_server_state_waiting_for_auth };
static const DBusAuthStateData server_state_waiting_for_data = { "WaitingForData",
                                                                 handle_server_state_waiting_for_data };
static const DBusAuthStateData server_state_waiting_for_begin = { "WaitingForBegin",
...
static dbus_bool_t handle_client_state_waiting_for_data(DBusAuth *auth, DBusAuthCommand command,
                                                        const DBusString *args);
static dbus_bool_t handle_client_state_waiting_for_ok(DBusAuth *auth, DBusAuthCommand command, const DBusString *args);
static dbus_bool_t handle_client_state_waiting_for_reject(DBusAuth *auth, DBusAuthCommand command,
                                                          const DBusString *args);
static dbus_bool_t handle_client_state_waiting_for_agree_unix_fd(DBusAuth *auth, DBusAuthCommand command,
                                                                 const DBusString *args);

static const DBusAuthStateData client_state_need_send_auth = { "NeedSendAuth", NULL };
static const DBusAuthStateData client_state_waiting_for_data = { "WaitingForData",
...
static const DBusAuthStateData client_state_waiting_for_reject = { "WaitingForReject",
 ...
 */
static dbus_bool_t sha1_compute_hash(DBusAuth *auth, int cookie_id, const DBusString *server_challenge,
                                     const DBusString *client_challenge, DBusString *hash)
{
    DBusString cookie;
    DBusString to_hash;
    dbus_bool_t retval;
    ...
    if (!_dbus_string_copy(server_challenge, 0, &to_hash, _dbus_string_get_length(&to_hash)))
        goto out_1;

    if (!_dbus_string_append(&to_hash, ":"))
        goto out_1;

    if (!_dbus_string_copy(client_challenge, 0, &to_hash, _dbus_string_get_length(&to_hash)))
        goto out_1;

    if (!_dbus_string_append(&to_hash, ":"))
        goto out_1;

    if (!_dbus_string_copy(&cookie, 0, &to_hash, _dbus_string_get_length(&to_hash)))
        goto out_1;
...
static dbus_bool_t sha1_handle_first_client_response(DBusAuth *auth, const DBusString *data)
{
   ...
   */
    ...
    if (!_dbus_credentials_add_from_user(auth->desired_identity, data, DBUS_CREDENTIALS_ADD_FLAGS_USER_DATABASE,
                                         &error)) {
   ...
   */
    ...
    if (!_dbus_string_copy(&auth->context, 0, &tmp2, _dbus_string_get_length(&tmp2)))
        goto out;
    ...
    if (!_dbus_string_hex_encode(&tmp, 0, &tmp2, _dbus_string_get_length(&tmp2)))
        goto out;
...
static dbus_bool_t sha1_handle_second_client_response(DBusAuth *auth, const DBusString *data)
{
   ...
   */
    ...
    if (!_dbus_string_copy_len(data, i, _dbus_string_get_length(data) - i, &client_hash, 0))
        goto out_2;
    ...
    if (!sha1_compute_hash(auth, auth->cookie_id, &auth->challenge, &client_challenge, &correct_hash))
        goto out_3;
    ...
    if (!_dbus_credentials_add_credentials(auth->authorized_identity, auth->desired_identity))
        goto out_3;

    /* Copy process ID from the socket credentials if it's there
   */
    if (!_dbus_credentials_add_credential(auth->authorized_identity, DBUS_CREDENTIAL_UNIX_PROCESS_ID,
                                          auth->credentials))
        goto out_3;
...
static dbus_bool_t handle_client_initial_response_cookie_sha1_mech(DBusAuth *auth, DBusString *response)
{
    ...
    if (!_dbus_string_hex_encode(&username, 0, response, _dbus_string_get_length(response)))
        goto out_0;
...
static dbus_bool_t handle_client_data_cookie_sha1_mech(DBusAuth *auth, const DBusString *data)
{
   ...
   */
    ...
    if (!_dbus_string_find_blank(data, 0, &i)) {
        if (send_error(auth, "Server did not send context/ID/challenge properly"))
            retval = TRUE;
        goto out_0;
    }
    ...
    if (!sha1_compute_hash(auth, val, &server_challenge, &client_challenge, &correct_hash))
        goto out_6;
    ...
    if (!_dbus_string_copy(&client_challenge, 0, &tmp, _dbus_string_get_length(&tmp)))
        goto out_6;

    if (!_dbus_string_append(&tmp, " "))
        goto out_6;

    if (!_dbus_string_copy(&correct_hash, 0, &tmp, _dbus_string_get_length(&tmp)))
        goto out_6;
...
static dbus_bool_t handle_server_data_external_mech(DBusAuth *auth, const DBusString *data)
{
   ...
   */
    if (_dbus_string_get_length(&auth->identity) == 0) {
        ...
        if (!_dbus_credentials_add_from_user(auth->desired_identity, &auth->identity, DBUS_CREDENTIALS_ADD_FLAGS_NONE,
                                             &error)) {
            if (dbus_error_has_name(&error, DBUS_ERROR_NO_MEMORY)) {
                dbus_error_free(&error);
                return FALSE;
    ...
    if (_dbus_credentials_are_superset(auth->credentials, auth->desired_identity)) {
       ...
       */
        ...
        if (!_dbus_credentials_add_credential(auth->authorized_identity, DBUS_CREDENTIAL_ADT_AUDIT_DATA_ID,
                                              auth->credentials))
            return FALSE;

        if (!_dbus_credentials_add_credential(auth->authorized_identity, DBUS_CREDENTIAL_UNIX_GROUP_IDS,
                                              auth->credentials))
            return FALSE;

        if (!_dbus_credentials_add_credential(auth->authorized_identity, DBUS_CREDENTIAL_LINUX_SECURITY_LABEL,
                                              auth->credentials))
            return FALSE;
...
static dbus_bool_t handle_client_initial_response_external_mech(DBusAuth *auth, DBusString *response)
{
   ...
   */
    ...
    if (!_dbus_string_hex_encode(&plaintext, 0, response, _dbus_string_get_length(response)))
        goto failed;
...
static const DBusAuthMechanismHandler all_mechanisms[] = {
    { "EXTERNAL", handle_server_data_external_mech, NULL, NULL, handle_server_shutdown_external_mech,
      handle_client_initial_response_external_mech, handle_client_data_external_mech, NULL, NULL,
      handle_client_shutdown_external_mech },
    { "DBUS_COOKIE_SHA1", handle_server_data_cookie_sha1_mech, NULL, NULL, handle_server_shutdown_cookie_sha1_mech,
      handle_client_initial_response_cookie_sha1_mech, handle_client_data_cookie_sha1_mech, NULL, NULL,
      handle_client_shutdown_cookie_sha1_mech },
    { "ANONYMOUS", handle_server_data_anonymous_mech, NULL, NULL, handle_server_shutdown_anonymous_mech,
      handle_client_initial_response_anonymous_mech, handle_client_data_anonymous_mech, NULL, NULL,
...
static dbus_bool_t send_data(DBusAuth *auth, DBusString *data)
{
    ...
    if (data == NULL || _dbus_string_get_length(data) == 0)
        ...
        if (!_dbus_string_hex_encode(data, 0, &auth->outgoing, _dbus_string_get_length(&auth->outgoing)))
            goto out;
...
static dbus_bool_t send_rejected(DBusAuth *auth)
{
    ...
    if (!_dbus_string_copy(&command, 0, &auth->outgoing, _dbus_string_get_length(&auth->outgoing)))
        goto nomem;
...
static dbus_bool_t handle_auth(DBusAuth *auth, const DBusString *args)
{
    if (_dbus_string_get_length(args) == 0) {
        ...
        if (auth->mech != NULL) {
            ...
            if (!process_data(auth, &hex_response, auth->mech->server_data_func))
                goto failed;
        } else {
...
static const DBusAuthCommandName auth_command_names[] = { { "AUTH", DBUS_AUTH_COMMAND_AUTH },
...
{
    return _dbus_string_replace_len(context, 0, _dbus_string_get_length(context), &auth->context, 0,
