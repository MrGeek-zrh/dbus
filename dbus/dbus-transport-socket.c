...
static void check_write_watch(DBusTransport *transport)
{
    ...
    _dbus_verbose("check_write_watch(): needed = %d on connection %p watch %p fd = %" DBUS_SOCKET_FORMAT
...
static dbus_bool_t exchange_credentials(DBusTransport *transport, dbus_bool_t do_reading, dbus_bool_t do_writing)
{
    ...
    if (!(transport->send_credentials_pending || transport->receive_credentials_pending)) {
        if (!_dbus_auth_set_credentials(transport->auth, transport->credentials))
            return FALSE;
    }
...
static dbus_bool_t do_authentication(DBusTransport *transport, dbus_bool_t do_reading, dbus_bool_t do_writing,
    ...
    while (!_dbus_transport_try_to_authenticate(transport) && _dbus_transport_get_is_connected(transport)) {
        ...
        if (transport->send_credentials_pending || transport->receive_credentials_pending) {
            _dbus_verbose("send_credentials_pending = %d receive_credentials_pending = %d\n",
                          transport->send_credentials_pending, transport->receive_credentials_pending);
            goto out;
        }
          ...
          _dbus_verbose ("message is %d bytes\n",
            ...
            if (socket_transport->message_bytes_written <= 0 && DBUS_TRANSPORT_CAN_SEND_UNIX_FD(transport)) {
                ...
                bytes_written = _dbus_write_socket_with_unix_fds_two(
                        socket_transport->fd, header, socket_transport->message_bytes_written,
...
#endif
            {
                if (socket_transport->message_bytes_written < header_len) {
                    bytes_written = _dbus_write_socket_two(socket_transport->fd, header,
                                                           socket_transport->message_bytes_written,
                                                           header_len - socket_transport->message_bytes_written, body,
                                                           0, body_len);
                } else {
                    bytes_written = _dbus_write_socket(
                            socket_transport->fd, body, (socket_transport->message_bytes_written - header_len),
...
again:
    ...
    if (_dbus_auth_needs_decoding(transport->auth)) {
        ...
        if (_dbus_string_get_length(&socket_transport->encoded_incoming) > 0)
            ...
            bytes_read = _dbus_read_socket(socket_transport->fd, &socket_transport->encoded_incoming,
                                           socket_transport->max_bytes_read_per_iteration);
...
static dbus_bool_t socket_connection_set(DBusTransport *transport)
{
    ...
    if (!_dbus_connection_add_watch_unlocked(transport->connection, socket_transport->write_watch))
        return FALSE;
...
static void socket_do_iteration(DBusTransport *transport, unsigned int flags, int timeout_milliseconds)
{
    ...
    _dbus_verbose(" iteration flags = %s%s timeout = %d read_watch = %p write_watch = %p fd = %" DBUS_SOCKET_FORMAT
                  "\n",
                  flags & DBUS_ITERATION_DO_READING ? "read" : "", flags & DBUS_ITERATION_DO_WRITING ? "write" : "",
                  timeout_milliseconds, socket_transport->read_watch, socket_transport->write_watch,
                  _dbus_socket_printable(socket_transport->fd));
...
static const DBusTransportVTable socket_vtable = { socket_finalize,     socket_handle_watch,
                                                   socket_disconnect,   socket_connection_set,
                                                   socket_do_iteration, socket_live_messages_changed,
 ...
 */
/**
 * @brief 为给定的 socket 创建并初始化一个 DBusTransport 对象
 *
 * 这个函数创建并初始化一个 `DBusTransportSocket` 对象，该对象用于处理与 socket 相关的传输。
 *
 * @param fd 给定的 socket 描述符,应该是客户端fd
 * @param server_guid 服务器 GUID
 * @param address 服务器地址
 * @returns 成功时返回指向 `DBusTransport` 的指针，失败时返回 `NULL`
 */
// 该函数主要处理与 socket 相关的传输初始化工作
DBusTransport *_dbus_transport_new_for_socket(DBusSocket fd, const DBusString *server_guid, const DBusString *address)
{
    // 定义一个指向 DBusTransportSocket 的指针
    DBusTransportSocket *socket_transport;
    // 定义一个无效的 DBusString
    DBusString invalid = _DBUS_STRING_INIT_INVALID;

    // 分配并初始化 DBusTransportSocket 对象
 ...
 */
DBusTransport *_dbus_transport_new_for_tcp_socket(const char *host, const char *port, const char *family,
                                                  const char *noncefile, DBusError *error)
{
    ...
    if (!_dbus_string_append(&address, "host=") || !_dbus_string_append(&address, host))
        goto error;

    if (!_dbus_string_append(&address, ",port=") || !_dbus_string_append(&address, port))
        goto error;
 ...
 */
DBusTransportOpenResult _dbus_transport_open_socket(DBusAddressEntry *entry, DBusTransport **transport_p,
                                                    DBusError *error)
{
    const char *method;
    dbus_bool_t isTcp;
