                                      ...
                                      const DBusString *server_guid, const DBusString *address)
{
   ...
   */
    ...
    _dbus_counter_set_notify(transport->live_messages, transport->max_live_messages_size,
...
static DBusTransportOpenResult _dbus_transport_open_autolaunch(DBusAddressEntry *entry, DBusTransport **transport_p,
                                                               DBusError *error)
{
    const char *method;
    ...
    if (transport->allow_anonymous || _dbus_credentials_get_unix_uid(auth_identity) == 0 ||
        ...
        if (_dbus_credentials_include(our_identity, DBUS_CREDENTIAL_WINDOWS_SID))
            ...
            _dbus_verbose("Client authorized as UID " DBUS_UID_FORMAT " matching our UID " DBUS_UID_FORMAT "\n",
                          _dbus_credentials_get_unix_uid(auth_identity), _dbus_credentials_get_unix_uid(our_identity));
        ...
        if (_dbus_credentials_include(our_identity, DBUS_CREDENTIAL_WINDOWS_SID))
            ...
            _dbus_verbose("Client authorized as UID " DBUS_UID_FORMAT " but our UID is " DBUS_UID_FORMAT
                          ", disconnecting\n",
                          _dbus_credentials_get_unix_uid(auth_identity), _dbus_credentials_get_unix_uid(our_identity));
        _dbus_transport_disconnect(transport);
        allow = FALSE;
 ...
 */
void _dbus_transport_set_unix_user_function(DBusTransport *transport, DBusAllowUnixUserFunction function, void *data,
...
void _dbus_transport_set_windows_user_function(DBusTransport *transport, DBusAllowWindowsUserFunction function,
                                               void *data, DBusFreeFunction free_data_function, void **old_data,
                                               DBusFreeFunction *old_free_data_function)
{
...
void _dbus_transport_get_stats(DBusTransport *transport, dbus_uint32_t *queue_bytes, dbus_uint32_t *queue_fds,
                               dbus_uint32_t *peak_queue_bytes, dbus_uint32_t *peak_queue_fds)
{
    if (queue_bytes != NULL)
