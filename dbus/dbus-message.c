...
{
    return (iter != NULL && iter->message == NULL && iter->changed_stamp == 0 && iter->iter_type == 0 &&
            iter->sig_refcount == 0);
}
                                          ...
                                          const char *method)
{
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER, DBUS_MESSAGE_TYPE_METHOD_CALL, destination,
                             path, iface, method, NULL)) {
        dbus_message_unref(message);
        return NULL;
    }
...
DBusMessage *dbus_message_new_method_return(DBusMessage *method_call)
{
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER, DBUS_MESSAGE_TYPE_METHOD_RETURN, sender, NULL,
                             NULL, NULL, NULL)) {
        dbus_message_unref(message);
        return NULL;
    }
 ...
 */
/**
 * @brief 创建一个新的 DBus 信号消息
 *
 * 这个函数用于创建一个新的 DBus 信号消息，并初始化消息头的路径、接口和信号名称。
 *
 * @param path 信号的路径
 * @param iface 信号的接口
 * @param name 信号的名称
 * @returns 如果成功创建消息，返回指向 DBusMessage 的指针；否则返回 NULL
 */
DBusMessage *dbus_message_new_signal(const char *path, const char *iface, const char *name)
{
    ...
    // 初始化消息头的路径、接口和信号名称
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER, DBUS_MESSAGE_TYPE_SIGNAL, NULL, path, iface,
                             name, NULL)) {
        // 如果初始化失败，释放消息
        dbus_message_unref(message);
        return NULL;
...
DBusMessage *dbus_message_new_error(DBusMessage *reply_to, const char *error_name, const char *error_message)
{
   ...
   */
    ...
    if (!_dbus_header_create(&message->header, DBUS_COMPILER_BYTE_ORDER, DBUS_MESSAGE_TYPE_ERROR, sender, NULL, NULL,
                             NULL, error_name)) {
        dbus_message_unref(message);
        return NULL;
    }
    ...
    while (type != DBUS_TYPE_INVALID) {
        if (dbus_type_is_basic(type)) {
            ...
            if (dbus_type_is_fixed(element_type) && element_type != DBUS_TYPE_UNIX_FD) {
                ...
                _dbus_warn("arrays of %s can't be appended with %s for now", _dbus_type_to_string(element_type),
                           _DBUS_FUNCTION_NAME);
                dbus_message_iter_abandon_container(&iter, &array);
                goto failed;
            }
...
dbus_bool_t dbus_message_iter_init(DBusMessage *message, DBusMessageIter *iter)
{
    ...
    _dbus_type_reader_init(&real->u.reader, _dbus_header_get_byte_order(&message->header), type_str, type_pos,
                           &message->body, 0);
...
char *dbus_message_iter_get_signature(DBusMessageIter *iter)
{
    ...
    if (!_dbus_string_append_len(&retstr, _dbus_string_get_const_data(sig) + start, len))
        goto oom;
...
void dbus_message_iter_init_append(DBusMessage *message, DBusMessageIter *iter)
{
   ...
   * due to OOM.
   */
    _dbus_type_writer_init_types_delayed(&real->u.writer, _dbus_header_get_byte_order(&message->header), &message->body,
...
static dbus_bool_t _dbus_message_iter_open_signature(DBusMessageRealIter *real)
{
    ...
    if (!_dbus_header_get_field_raw(&real->message->header, DBUS_HEADER_FIELD_SIGNATURE, &current_sig,
                                    &current_sig_pos))
        current_sig = NULL;
...
static dbus_bool_t _dbus_message_iter_close_signature(DBusMessageRealIter *real)
{
    ...
    if (!_dbus_header_set_field_basic(&real->message->header, DBUS_HEADER_FIELD_SIGNATURE, DBUS_TYPE_SIGNATURE,
                                      &v_STRING))
        retval = FALSE;
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t dbus_message_iter_append_fixed_array(DBusMessageIter *iter, int element_type, const void *value,
                                                 int n_elements)
{
    DBusMessageRealIter *real = (DBusMessageRealIter *)iter;
    dbus_bool_t ret;
 ...
 * @returns #FALSE if not enough memory
 */
dbus_bool_t dbus_message_iter_open_container(DBusMessageIter *iter, int type, const char *contained_signature,
                                             DBusMessageIter *sub)
{
    DBusMessageRealIter *real = (DBusMessageRealIter *)iter;
    DBusMessageRealIter *real_sub = (DBusMessageRealIter *)sub;
   ...
   * valid-looking on failure */
    ...
    _dbus_return_val_if_fail((type == DBUS_TYPE_STRUCT && contained_signature == NULL) ||
   ...
   */
    ...
    _dbus_return_val_if_fail((type == DBUS_TYPE_ARRAY && contained_signature &&
...
void dbus_message_iter_abandon_container_if_open(DBusMessageIter *iter, DBusMessageIter *sub)
{
   ...
   * call failed, or if we reached result = TRUE and fell through. */
    if (_dbus_message_real_iter_is_zeroed(real) && _dbus_message_real_iter_is_zeroed(real_sub))
        return;
...
dbus_bool_t dbus_message_set_container_instance(DBusMessage *message, const char *object_path)
{
    ...
    return set_or_delete_string_field(message, DBUS_HEADER_FIELD_CONTAINER_INSTANCE, DBUS_TYPE_OBJECT_PATH,
                                      object_path);
}
...
const char *dbus_message_get_container_instance(DBusMessage *message)
{
    ...
    v = NULL; /* in case field doesn't exist */
    _dbus_header_get_field_basic(&message->header, DBUS_HEADER_FIELD_CONTAINER_INSTANCE, DBUS_TYPE_OBJECT_PATH,
                                 (void *)&v);
    return v;
}
...
#endif
        _dbus_list_clear_full(&loader->messages, (DBusFreeFunction)dbus_message_unref);
 ...
 */
void _dbus_message_loader_get_buffer(DBusMessageLoader *loader, DBusString **buffer, int *max_to_read,
       ...
       * will never go away. */
        ...
        while (remain > 0) {
           ...
           * DBUS_MINIMUM_HEADER_SIZE for now; then we'll know. */
            ...
            if (!_dbus_header_have_message_untrusted(loader->max_message_size, &validity, &byte_order,
                                                     &fields_array_len, &header_len, &body_len, &loader->data, offset,
                                                     remain)) {
 ...
 */
static dbus_bool_t load_message(DBusMessageLoader *loader, DBusMessage *message, int byte_order, int fields_array_len,
                                int header_len, int body_len)
{
  ...
  _dbus_verbose_bytes_of_string (&loader->data, 0, header_len /* + body_len */);
    ...
    if (!_dbus_header_load(&message->header, mode, &validity, byte_order, fields_array_len, header_len, body_len,
                           &loader->data)) {
    ...
    if (mode != DBUS_VALIDATION_MODE_WE_TRUST_THIS_DATA_ABSOLUTELY) {
       ...
       */
        validity = _dbus_validate_body_with_reason(type_str, type_pos, byte_order, NULL, &loader->data, header_len,
                                                   body_len);
        if (validity != DBUS_VALID) {
...
dbus_bool_t _dbus_message_loader_queue_messages(DBusMessageLoader *loader)
{
    while (!loader->corrupted && _dbus_string_get_length(&loader->data) >= DBUS_MINIMUM_HEADER_SIZE) {
        ...
        if (_dbus_header_have_message_untrusted(loader->max_message_size, &validity, &byte_order, &fields_array_len,
                                                &header_len, &body_len, &loader->data, 0,
...
dbus_bool_t dbus_message_set_data(DBusMessage *message, dbus_int32_t slot, void *data, DBusFreeFunction free_data_func)
{
    ...
    retval = _dbus_data_slot_list_set(&slot_allocator, &message->slot_list, slot, data, free_data_func, &old_free_func,
                                      &old_data);
...
int dbus_message_demarshal_bytes_needed(const char *buf, int len)
{
    ...
    validity = DBUS_VALID;
    have_message = _dbus_header_have_message_untrusted(DBUS_MAXIMUM_MESSAGE_LENGTH, &validity, &byte_order,
                                                       &fields_array_len, &header_len, &body_len, &str, 0, len);
    _dbus_string_free(&str);
   ...
   * of the single complete type of the item reader points to */
    ...
    if (type == DBUS_TYPE_ARRAY) {
       ...
       * the copy of array_reader */
        ...
        if (!_dbus_type_writer_recurse(&variant_writer, type, &contained_signature, 1, &array_writer))
            goto oom;

        if (!_dbus_type_writer_write_reader(&array_writer, &real_array_reader->u.reader))
            goto oom;
       ...
       * copy of inner_reader */
        ...
        if (!_dbus_type_writer_recurse(&variant_writer, type, NULL, 0, &inner_writer))
            goto oom;

        if (!_dbus_type_writer_write_reader(&inner_writer, &real_inner_reader->u.reader))
            goto oom;
