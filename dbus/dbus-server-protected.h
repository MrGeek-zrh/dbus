...
struct DBusServer {
    ...
    dbus_bool_t
            published_address; /**< flag which indicates that server has published its bus address. 指示服务器是否已发布其总线地址的标志。 */
    ...
    DBusNewConnectionFunction
            new_connection_function; /**< Callback to invoke when a new connection is created. 创建新连接时调用的回调函数。 */
    ...
    unsigned int
            have_server_lock : 1; /**< Does someone have the server mutex locked. 指示是否有线程持有服务器的互斥锁。 */
#endif
};
...
DBusServerListenResult _dbus_server_listen_platform_specific(DBusAddressEntry *entry, DBusServer **server_p,
                                                             DBusError *error);
