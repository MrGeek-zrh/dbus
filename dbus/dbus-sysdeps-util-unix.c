...
static dbus_bool_t _dbus_write_pid_file(const DBusString *filename, unsigned long pid, DBusError *error)
{
    ...
    if (fd < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to open \"%s\": %s", cfilename,
    ...
    if (fprintf(f, "%lu\n", pid) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to write to \"%s\": %s", cfilename,
                       _dbus_strerror(errno));
    ...
    if (fclose(f) == EOF) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to close \"%s\": %s", cfilename,
                       _dbus_strerror(errno));
        return FALSE;
    }
 ...
 */
dbus_bool_t _dbus_write_pid_to_file_and_pipe(const DBusString *pidfile, DBusPipe *print_pid_pipe,
                                             dbus_pid_t pid_to_write, DBusError *error)
{
    if (pidfile) {
...
dbus_bool_t _dbus_change_to_daemon_user(const char *user, DBusError *error)
{
   ...
   */
    if (setgroups(0, NULL) < 0)
        _dbus_warn("Failed to drop supplementary groups: %s", _dbus_strerror(errno));

    /* Set GID first, or the setuid may remove our permission
   * to change the GID
   */
    if (setgid(gid) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to set GID to %lu: %s", gid,
                       _dbus_strerror(errno));
        return FALSE;
    }

    if (setuid(uid) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to set UID to %lu: %s", uid,
                       _dbus_strerror(errno));
        return FALSE;
    }
...
dbus_bool_t _dbus_rlimit_raise_fd_limit(DBusError *error)
{
   ...
   * to 1024 and rlim_max to 4096. */
    ...
    if (setrlimit(RLIMIT_NOFILE, &lim) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to set fd limit to %lu: %s",
...
dbus_bool_t _dbus_rlimit_restore_fd_limit(DBusRLimit *saved, DBusError *error)
{
    if (setrlimit(RLIMIT_NOFILE, &saved->lim) < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to restore old fd limit: %s",
                       _dbus_strerror(errno));
        return FALSE;
    }
...
DBusDirIter *_dbus_directory_open(const DBusString *filename, DBusError *error)
{
    ...
    if (d == NULL) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to read directory \"%s\": %s", filename_c,
                       _dbus_strerror(errno));
        return NULL;
    }
...
again:
    ...
    if (!ent) {
        ...
        if (err != 0)
            dbus_set_error(error, _dbus_error_from_errno(err), "%s", _dbus_strerror(err));
...
dbus_bool_t _dbus_command_for_pid(unsigned long pid, DBusString *str, int max_len, DBusError *error)
{
    ...
    if (fd < 0) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to open \"%s\": %s",
                       _dbus_string_get_const_data(&path), _dbus_strerror(errno));
        goto fail;
    }

    if (!_dbus_read(fd, &cmdline, max_len)) {
        dbus_set_error(error, _dbus_error_from_errno(errno), "Failed to read from \"%s\": %s",
                       _dbus_string_get_const_data(&path), _dbus_strerror(errno));
        _dbus_close(fd, NULL);
        goto fail;
    }
...
static dbus_bool_t ensure_owned_directory(const char *label, const DBusString *string, dbus_bool_t create,
                                          DBusError *error)
{
   ...
   */
    ...
    if (stat(dir, &buf) != 0) {
        ...
        dbus_set_error(error, _dbus_error_from_errno(saved_errno), "%s \"%s\" not available: %s", label, dir,
    ...
    if (buf.st_uid != geteuid()) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "%s \"%s\" is owned by uid %ld, not our uid %ld", label, dir,
                       (long)buf.st_uid, (long)geteuid());
        return FALSE;
    }
   ...
   * as well check opportunistically. */
    if ((S_IWOTH | S_IWGRP) & buf.st_mode) {
        dbus_set_error(error, DBUS_ERROR_FAILED, "%s \"%s\" can be written by others (mode 0%o)", label, dir,
                       buf.st_mode);
        return FALSE;
    }
...
dbus_bool_t _dbus_get_standard_session_servicedirs(DBusList **dirs)
{
   ...
   */
    ...
    if (!_dbus_split_paths_and_append(&servicedir_path, DBUS_UNIX_STANDARD_SESSION_SERVICEDIR, dirs))
        goto oom;
